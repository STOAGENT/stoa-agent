...
# xAI's public API is OpenAI-compatible for the endpoints STOA commonly
# uses. The Responses endpoint is included because STOA' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via STOA-managed OAuth credentials."""

    auth_hint = "stoa auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`stoa auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`stoa auth reset xai-oauth` or re-authenticate with "
                    "`stoa auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `stoa auth add xai-oauth --type oauth`."
            )
