...
Reads the user's Nous OAuth state from ``~/.stoa/auth.json`` through the
...
:func:`stoa_cli.auth.resolve_nous_runtime_credentials`.
"""
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `stoa login nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `stoa login nous` to re-authenticate."
                )
    ...
    # to stoa_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
