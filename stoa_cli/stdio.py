...
on Japanese locales, etc.).  STOA's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``stoa_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. STOA is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``STOA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("STOA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the STOA-managed Git install directories so
    ...
    # already-running shells don't see the change — which means stoa
    ...
    # install without STOA-managed PortableGit).
    ...
    # stdin is re-configured for completeness; STOA's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and STOA's
    ``stoa config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching STOA (User env var in Windows Settings, or
    export in a PowerShell profile) and STOA picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known STOA-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\stoa\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``stoa`` from right after install) retain their old PATH.

    Any subprocess STOA spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known STOA-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "stoa", "git", "cmd"),
        os.path.join(local_appdata, "stoa", "git", "bin"),
        os.path.join(local_appdata, "stoa", "git", "usr", "bin"),
        # STOA venv Scripts directory — host of the stoa.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes stoa, but harmless to include.
        os.path.join(local_appdata, "stoa", "stoa-agent", "venv", "Scripts"),
