"""
Profile management for multiple isolated STOA instances.

Each profile is a fully independent STOA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.stoa/profiles/<name>/`` by default.

The "default" profile is ``~/.stoa`` itself — backward compatible,
zero migration needed.

Usage::

    stoa profile create coder          # fresh profile + bundled skills
    stoa profile create coder --clone  # also copy config, .env, SOUL.md, skills
    stoa profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    stoa -p coder chat                 # or via flag
    stoa profile use coder             # set as sticky default
    stoa profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See stoa_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `stoa profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `stoa
...
# `stoa skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         STOA infrastructure directories that only the default profile
         (``~/.stoa``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_stoa_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.stoa) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "stoa-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "stoa", "default", "test", "tmp", "root", "sudo",
})

# STOA subcommands that cannot be used as profile names/aliases
_STOA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the stoa root, NOT to the current STOA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where STOA_HOME points outside
    ``~/.stoa``, profiles live under ``STOA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_stoa_home() / "profiles"


def _get_default_stoa_home() -> Path:
    """Return the default (pre-profile) STOA_HOME path.

    In standard deployments this is ``~/.stoa``.
    In Docker/custom deployments where STOA_HOME is outside ``~/.stoa``
    (e.g. ``/opt/data``), returns STOA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_stoa_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the STOA installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its STOA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_stoa_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, stoa subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _STOA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a stoa subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``stoa profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing STOA config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_stoa_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``stoa update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.stoa)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.stoa/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `stoa update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`stoa profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `stoa update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `stoa profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches STOA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``stoa profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "STOA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.stoa).\n"
            "To remove everything, use: stoa uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set STOA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("STOA_HOME")
    try:
        os.environ["STOA_HOME"] = str(profile_dir)
        from stoa_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["STOA_HOME"] = old_home
        elif "STOA_HOME" in os.environ:
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.stoa/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: stoa profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from STOA_HOME.

    Returns ``"default"`` if STOA_HOME is not set or points to ``~/.stoa``.
    Returns the profile name if STOA_HOME points into ``~/.stoa/profiles/<name>``.
    Returns ``"custom"`` if STOA_HOME is set to an unrecognized path.
    """
    from stoa_constants import get_stoa_home
    stoa_home = get_stoa_home()
    resolved = stoa_home.resolve()

    default_resolved = _get_default_stoa_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.stoa itself — its parent is ~/ and its
        # directory name is ".stoa", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: stoa profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.stoa itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.stoa). "
            "Specify a different name: stoa profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="stoa_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"stoa.{old_name}"
    new_host = f"stoa.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_stoa_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a STOA_HOME path string.

    Called early in the CLI entry point, before any stoa modules
    are imported, to set the STOA_HOME environment variable.
    """
