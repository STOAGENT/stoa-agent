"""
STOA Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.stoa/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove STOA PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing stoa-agent or stoa PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# STOA Agent" comment and following line
                if '# STOA Agent' in line or '# stoa-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('stoa' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing stoa
                if 'stoa' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the stoa wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "stoa",
        Path("/usr/local/bin/stoa"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains stoa_cli reference)
                content = wrapper.read_text()
                if 'stoa_cli' in content or 'stoa-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``stoa gateway run`` processes
    ...
    try:
        from stoa_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from stoa_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``STOA_HOME`` and ``STOA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\stoa\git\cmd``, ``%LOCALAPPDATA%\stoa\git\bin``,
#      ``%LOCALAPPDATA%\stoa\git\usr\bin``, ``%LOCALAPPDATA%\stoa\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\stoa\git\`` and Node to
#      ``%LOCALAPPDATA%\stoa\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``stoa dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\stoa\gateway-service\`` and sometimes
...
def _stoa_path_markers(stoa_home: Path) -> list[str]:
    """Path-entry substrings that identify STOA-owned User-PATH entries."""
    root = str(stoa_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare stoa-agent install dir.
    markers = [root + "\\stoa-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if STOA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "stoa".  We don't want to catch
    # unrelated entries like "cstoa-foo" or "ephermeral", so we look for
    # backslash-stoa as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(stoa_home: Path) -> list[str]:
    """Strip STOA-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _stoa_path_markers(stoa_home)
            kept: list[str] = []
...
def remove_stoa_env_vars_windows() -> list[str]:
    """Delete STOA_HOME and STOA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("STOA_HOME", "STOA_GIT_BASH_PATH"):
                try:
...
def _is_default_stoa_home(stoa_home: Path) -> bool:
    """Return True when ``stoa_home`` points at the default (non-profile) root."""
    try:
        from stoa_constants import get_default_stoa_root
        return stoa_home.resolve() == get_default_stoa_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its STOA_HOME directory.

    We shell out to ``stoa -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current STOA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m stoa_cli.main` so we don't depend on a `stoa`
    #    wrapper that may be half-removed mid-uninstall.
    stoa_invocation = [_sys.executable, "-m", "stoa_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                stoa_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's STOA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.stoa/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.stoa/ for future reinstall
    """
    project_root = get_project_root()
    stoa_home = get_stoa_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie STOA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_stoa_home(stoa_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⁂ STOA Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {stoa_home / 'config.yaml'}")
    print(f"  Secrets: {stoa_home / '.env'}")
    print(f"  Data:    {stoa_home / 'cron/'}, {stoa_home / 'sessions/'}, {stoa_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their STOA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL STOA data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the STOA code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in stoa_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\stoa\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(stoa_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No STOA-owned PATH entries in User environment")

        log_info("Removing STOA_HOME / STOA_GIT_BASH_PATH User env vars...")
        removed_env = remove_stoa_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No STOA-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing stoa command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.stoa/, just remove the stoa-agent subdir
            if stoa_home in project_root.parents or project_root.parent == stoa_home:
    ...
    #     under STOA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(stoa_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(stoa_home)
    ...
    # 5. Optionally remove ~/.stoa/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile STOA_HOME dirs live under
        ...
        try:
            if stoa_home.exists():
                shutil.rmtree(stoa_home)
                log_success(f"Removed {stoa_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {stoa_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {stoa_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/STOAGENT/stoa-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/STOAGENT/stoa-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using STOA Agent! ⁂")
    print()
