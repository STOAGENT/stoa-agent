    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_stoa_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
...
_stoa_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_stoa_profiles)" -- "$cur"))
        return
    fi
...
_stoa_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.stoa/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.stoa/profiles 2>/dev/null)}}")
    fi
...
_stoa() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_stoa_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "    echo default",
        "    if test -d $HOME/.stoa/profiles",
        "        ls $HOME/.stoa/profiles 2>/dev/null",
        "    end",
        "end",
        "",
