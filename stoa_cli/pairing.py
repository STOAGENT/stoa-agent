...
Usage:
    stoa pairing list              # Show all pending + approved users
    stoa pairing approve <platform> <code>  # Approve a pairing code
    stoa pairing revoke <platform> <user_id> # Revoke user access
    stoa pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle stoa pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: stoa pairing {list|approve|revoke|clear-pending}")
        print("Run 'stoa pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.stoa/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'stoa pairing list' to see pending codes.\n")
