...
Saves per-platform tool configuration to ~/.stoa/config.yaml under
...
from stoa_cli.nous_subscription import (
...
from stoa_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _STOA_CORE_TOOLS (available at runtime if enabled),
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from stoa_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from stoa_cli.plugins import discover_plugins, get_plugin_toolsets
...
from stoa_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. STOA_CUA_DRIVER_VERSION is an
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("STOA_CUA_DRIVER_CMD", "").strip() or "cua-driver"
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_stoa_home()}/stoa-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in stoa-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.stoa/config.yaml")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "stoa-cli" (which include all _STOA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "stoa-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://stoax.xyz/docs/user-guide/features/tools", Colors.DIM))
    print()
