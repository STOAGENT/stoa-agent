"""
Configuration management for STOA Agent.

Config files are stored in ~/.stoa/ for easy access:
- ~/.stoa/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.stoa/.env         - API keys and secrets

This module provides:
- stoa config          - Show current configuration
- stoa config edit     - Open config in editor
- stoa config set      - Set a specific value
- stoa config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.stoa/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``stoa logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  stoa config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `stoa plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "STOA_LANGFUSE_ENV",
    "STOA_LANGFUSE_RELEASE",
    "STOA_LANGFUSE_SAMPLE_RATE",
    "STOA_LANGFUSE_MAX_CHARS",
    "STOA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from stoa_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("STOA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_stoa_home() / ".managed"
...
def is_managed() -> bool:
    """Check if STOA is running in package-manager-managed mode.

    Two signals: the STOA_MANAGED env var (set by the systemd service),
    or a .managed marker file in STOA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade stoa-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how STOA was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.stoa/.install_method`` file (written by installers)
    2. STOA_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_stoa_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.stoa/.install_method."""
    stamp = get_stoa_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade stoa-agent"
    if method == "docker":
        return "docker pull nousresearch/stoa-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade stoa-agent"
        return "pip install --upgrade stoa-agent"
    return "stoa update"
...
def format_managed_message(action: str = "modify this STOA installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("STOA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this STOA installation is managed by NixOS "
            f"(STOA_MANAGED={env_hint}).\n"
            "Edit services.stoa-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this STOA installation is managed by Homebrew "
            f"(STOA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade stoa-agent"
        )

    return (
        f"Cannot {action}: this STOA installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall STOA."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from STOA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, stoa_bin
    or None if container mode is not active, we're already inside the
    container, or STOA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("STOA_DEV") == "1":
        return None
    ...
    container_name = info.get("container_name", "stoa-agent")
    exec_user = info.get("exec_user", "stoa")
    stoa_bin = info.get("stoa_bin", "/data/current-package/bin/stoa")
...
# Re-export from stoa_constants — canonical definition lives there.
from stoa_constants import get_stoa_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_stoa_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_stoa_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the stoa group can
    share state with the gateway service.

    The mode can be overridden via the STOA_HOME_MODE environment variable
    (e.g. STOA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse STOA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("STOA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When STOA runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("STOA_CONTAINER") or os.environ.get("STOA_SKIP_CHMOD"):
        return True
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into STOA_HOME if the user doesn't have one yet."""
...
def ensure_stoa_home():
    """Ensure ~/.stoa directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_stoa_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_stoa_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"STOA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the STOA-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, STOA auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), STOA sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when STOA
        ...
        # bundled STOA image, which drops to the `stoa` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, STOA sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and STOA should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``stoa chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.stoa/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: stoa sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, STOA truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``stoa kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``stoa kanban decompose`` and the kanban
        ...
        # ``stoa profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `stoa model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `stoa --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `stoa -c` muscle memory.  Default off so existing
        # users aren't surprised.  STOA_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/stoa`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.stoa/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.stoa/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, STOA feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before STOA auto-pauses the goal and
    ...
    # always goes to ~/.stoa/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${STOA_SKILL_DIR} and ${STOA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `stoa curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.stoa/skills/ into
        # ~/.stoa/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `stoa curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for stoa-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *STOA Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (STOA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.stoa/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or STOA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `stoa doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``stoa_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow STOA to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via STOA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `stoa -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `stoa` launches with
        ...
        # decomposition is manual via `stoa kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     stoa-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.stoa/logs/.
    ...
    # update model picker lists without shipping a stoa-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://stoa-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `stoa model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.stoa/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.stoa/sessions/session_{sid}.json`` on every turn
    ...
    # ``stoa update`` behaviour.
    "updates": {
        # Run a full ``stoa backup``-style zip of STOA_HOME before every
        # ``stoa update``.  Backups land in ``<STOA_HOME>/backups/`` and
        # can be restored with ``stoa import <path>``.  Off by default —
        # on large STOA_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<STOA_HOME>/lsp/bin/`` on first use.
    ...
    # `stoa tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.stoa/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.stoa/.env (or your shell) and never in config.yaml.
            ...
            # ~/.stoa/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'stoa model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the STOA auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @stoa:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. STOA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #stoa)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: stoa-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'stoa-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote STOA API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote STOA API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote STOA API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # STOA_TOOL_PROGRESS and STOA_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.stoa.config`` entries, then checks
    ...
    try:
        ...
        # should never break `stoa update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older STOA versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``stoa_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'stoa setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — STOA won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'stoa doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("STOA_HOME", "~/.stoa")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("STOA_TOOL_PROGRESS")
            old_mode = get_env_value("STOA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from STOA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from STOA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("STOA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from STOA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$STOA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_stoa_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `stoa plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `stoa curator status` has no
    ...
    #   3. Creates `~/.stoa/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_stoa_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_stoa_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: stoa config set <key> <value>")
    ...
    # metadata.stoa.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _stoa_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.stoa/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.stoa/config.yaml.
    ...
    (which change ``STOA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — stoa auth) — OpenAI Codex
#   nous         (OAuth — stoa auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.stoa/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.stoa/.env.
    ...
    interactive menu render (`stoa tools`, `stoa setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `stoa tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real STOA env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.stoa/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.stoa/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.stoa/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.stoa/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to STOA — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known STOA vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.stoa/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ STOA Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  stoa config edit     # Edit config file", Colors.DIM))
    print(color("  stoa config set <key> <value>", Colors.DIM))
    print(color("  stoa setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: stoa config set <key> <value>")
            print()
            print("Examples:")
            print("  stoa config set model anthropic/claude-sonnet-4")
            print("  stoa config set terminal.backend docker")
            print("  stoa config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'stoa config migrate' to add them")
        ...
        print("  stoa config           Show current configuration")
        print("  stoa config edit      Open config in editor")
        print("  stoa config set <key> <value>   Set a config value")
        print("  stoa config check     Check for missing/outdated config")
        print("  stoa config migrate   Update config with new options")
        print("  stoa config path      Show config file path")
        print("  stoa config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``stoa config`` UI so users
