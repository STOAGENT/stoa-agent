"""Migrate STOA' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads STOA' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  STOA mcp_servers.<n>.command/args/env  → codex stdio transport
  STOA mcp_servers.<n>.url/headers       → codex streamable_http transport
  STOA mcp_servers.<n>.timeout           → codex tool_timeout_sec
  STOA mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec
...
What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. STOA' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in STOA config.")
...
# STOA keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # STOA' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, stoa_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one STOA MCP server config to the codex inline-table dict
    ...
    if not isinstance(stoa_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(stoa_cfg.get("command"))
    has_url = bool(stoa_cfg.get("url"))
    ...
    for key in stoa_cfg:
        ...
        elif key not in _KNOWN_STOA_KEYS:
            skipped.append(f"{key} (unknown STOA key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (STOA_HOME, PYTHONPATH) could
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert STOA' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before STOA' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior STOA
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="stoa-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a STOA_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed stoa-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.stoa`` symlink that happens to live under ``/private/var/folders``
...
def _build_stoa_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches STOA' own
    ...
    so a stoa installed under /opt/, /usr/local/, or a venv all work.
    STOA_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # STOA_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_stoa_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever STOA_HOME its launcher sets
    ...
    # — that would override the launcher's STOA_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("STOA_HOME", tmp_path) would
    ...
    stoa_home = os.environ.get("STOA_HOME") or ""
    if stoa_home and _looks_like_test_tempdir(stoa_home):
        stoa_home = ""
    if stoa_home:
        env["STOA_HOME"] = stoa_home
    # PYTHONPATH passes through so a worktree-launched stoa finds the
    ...
    env["STOA_QUIET"] = "1"
    env["STOA_REDACT_SECRETS"] = env.get("STOA_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_stoa_tools: bool = True,
) -> MigrationReport:
    """Translate STOA mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        stoa_config: full ~/.stoa/config.yaml dict
        ...
        expose_stoa_tools: when True (default), register STOA' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into STOA for tools
    ...
    stoa_servers = (stoa_config or {}).get("mcp_servers") or {}
    if not isinstance(stoa_servers, dict):
        report.errors.append(
            "mcp_servers in STOA config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in stoa_servers.items():
    ...
    # Inject STOA' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into STOA for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/stoa_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_stoa_tools:
        translated["stoa-tools"] = _build_stoa_tools_mcp_entry()
        if "stoa-tools" not in report.migrated:
            report.migrated.append("stoa-tools")
