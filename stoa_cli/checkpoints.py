"""`stoa checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.stoa/checkpoints/``.  Actions:

    stoa checkpoints               # same as `status`
    stoa checkpoints status        # total size, project count, breakdown
    stoa checkpoints list          # per-project checkpoint counts + workdir
    stoa checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    stoa checkpoints clear [-f]    # nuke the entire base (asks first)
    stoa checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    stoa checkpoints
    stoa checkpoints prune --retention-days 3 --max-size-mb 200
    stoa checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: stoa checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``stoa checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `stoa checkpoints` → status
