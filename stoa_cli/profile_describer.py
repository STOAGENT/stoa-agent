...
Used by ``stoa profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``stoa_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the STOA Agent kanban board.
...
Rules:
  ...
  - Never write "STOA Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_stoa_home())
        else:
