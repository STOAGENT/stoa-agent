"""CLI handlers for ``stoa secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``stoa_cli.main`` as part of building the top-level
    ``stoa secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every STOA process."
    )
    console.print(
        "  Status:  [cyan]stoa secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]stoa secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]stoa secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]stoa secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — STOA will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`stoa secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]stoa[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
