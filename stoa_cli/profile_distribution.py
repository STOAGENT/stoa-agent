"""Profile distributions — shareable, packaged STOA profiles via git.

A distribution is a STOA profile published as a git repository (or
...
* ``stoa profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``stoa skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``stoa profile``, not a parallel tree):

    stoa profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "stoa-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            stoa_requires=str(data.get("stoa_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.stoa_requires:
            out["stoa_requires"] = self.stoa_requires
        if self.author:
...
def check_stoa_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires STOA {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``stoa profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a STOA profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a STOA distribution."
        )

    # Version check up-front so we fail fast
    check_stoa_requires(manifest.stoa_requires, stoa_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.stoa).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="stoa_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `stoa profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`stoa profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="stoa_dist_update_") as tmp:
        plan = plan_install(
