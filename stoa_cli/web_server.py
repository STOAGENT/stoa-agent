"""
STOA Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m stoa_cli.main web          # Start on http://127.0.0.1:9119
    python -m stoa_cli.main web --port 8080
"""
...
try:
    ...
    # running `stoa dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["STOA_WEB_DIST"]) if "STOA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="STOA Agent", version=__version__)
...
# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``stoa dashboard --tui``
# or STOA_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.stoa/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_stoa_home() / "logs"
...
def _spawn_stoa_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``hermes <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``stoa_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "stoa_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "STOA_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``stoa gateway restart`` in the background."""
    try:
        proc = _spawn_stoa_action(["gateway", "restart"], "gateway-restart")
...
async def update_hermes():
    """Kick off ``stoa update`` in the background."""
    try:
        proc = _spawn_stoa_action(["update"], "hermes-update")
    except Exception as exc:
...
# in stoa_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.stoa/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``stoa auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.stoa/.anthropic_oauth.json`` — STOA-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_stoa_oauth_credentials,
            read_claude_code_credentials,
            _STOA_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_stoa_oauth_credentials = None  # type: ignore
        _STOA_OAUTH_FILE = None  # type: ignore

    stoa_creds = None
    if read_stoa_oauth_credentials:
        try:
            stoa_creds = read_stoa_oauth_credentials()
        except Exception:
            stoa_creds = None
    if stoa_creds and stoa_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "stoa_pkce",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    when they also have a separate STOA-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "stoa auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "stoa auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "stoa auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "stoa auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "stoa auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from stoa_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("stoa_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same STOA-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _STOA_OAUTH_FILE
            if _STOA_OAUTH_FILE.exists():
                _STOA_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from stoa_cli.auth import clear_provider_auth
...
#          → persists to ~/.stoa/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``stoa auth add anthropic``.
    """
    from agent.anthropic_adapter import _STOA_OAUTH_FILE
    payload = {
    ...
    _STOA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _STOA_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("STOA_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from stoa_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from stoa_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_stoa_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from stoa_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from stoa_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, STOA_HOME)."""
    from stoa_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process STOA_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_stoa_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "stoa setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from stoa_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from stoa_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from stoa_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``STOA_TUI_RESUME`` env var —
    matching what ``stoa_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``STOA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from stoa_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("STOA_TUI_DISABLE_MOUSE", "1")
    env.setdefault("STOA_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["STOA_TUI_RESUME"] = resume

    if sidecar_url:
        env["STOA_TUI_SIDECAR_URL"] = sidecar_url
...
# STOA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__STOA_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        token_script = (
            f'<script>window.__STOA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__STOA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__STOA_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.stoa/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.stoa/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_stoa_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.stoa/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as stoa_cli.plugins):
    1. User plugins:    ~/.stoa/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.stoa/plugins/  (only if STOA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from stoa_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_stoa_home() / "plugins", "user"),
    ...
    # ``stoa_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("STOA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".stoa" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from stoa_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_stoa_home() / "plugins").resolve()
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from stoa_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from stoa_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from stoa_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.stoa/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"stoa_dashboard_plugin_{plugin['name']}"
