"""
Status command for stoa CLI.

Shows the status of all STOA Agent components.
"""
...
from stoa_cli.auth import AuthError, resolve_provider
from stoa_cli.colors import Colors, color
from stoa_cli.config import get_env_path, get_env_value, get_stoa_home, load_config
from stoa_cli.models import provider_label
from stoa_cli.nous_subscription import get_nous_subscription_features
from stoa_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``stoa config``'s
...
def show_status(args):
    """Show status of all STOA Agent components."""
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: stoa auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: stoa model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: stoa auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: stoa auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: stoa model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from stoa_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'stoa-agent[vercel]')"
    ...
    try:
        from stoa_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_stoa_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_stoa_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'stoa doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'stoa setup' to configure", Colors.DIM))
    print()
