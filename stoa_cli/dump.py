"""
Dump command for stoa CLI.

Outputs a compact, plain-text summary of the user's STOA setup
...
from stoa_cli.config import get_stoa_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``stoa dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from stoa_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(stoa_home: Path) -> int:
...
def _cron_summary(stoa_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = stoa_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"stoa_home:      {display_stoa_home()}")
    ...
    toolsets = config.get("toolsets", ["stoa-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(stoa_home)}")
    lines.append(f"  skills:             {_count_skills(stoa_home)}")
