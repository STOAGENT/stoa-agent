...
# when stoa_constants is cached from a pre-update version during `hermes update`.
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.stoa`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.stoa/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _setup_weixin():
    ...
    print_info("  3. Hermes will store the returned account_id/token in ~/.stoa/.env.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print("  nohup hermes gateway run > ~/.stoa/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup hermes gateway run > ~/.stoa/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'hermes gateway run'         # persistent via tmux")
                    print("  nohup hermes gateway run > ~/.stoa/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
