...
Pure display functions with no STOACLI state dependency.
"""
...
STOA_AGENT_LOGO = f"""[bold #FFD700]███████╗████████╗ ██████╗  █████╗ [/]
[bold #FFC800]██╔════╝╚══██╔══╝██╔═══██╗██╔══██╗[/]
[bold #FFB900]███████╗   ██║   ██║   ██║███████║[/]
[bold #FFAA00]╚════██║   ██║   ██║   ██║██╔══██║[/]
[bold #FF9B00]███████║   ██║   ╚██████╔╝██║  ██║[/]
[bold #FF8C00]╚══════╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝[/]
[dim #DAA520]      A G E N T  ·  v{VERSION}[/]
[#FFBF00]  • • • • • •[/]
...
# (e.g. nix-built stoa — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/STOAGENT/stoa-agent.git"
...
def _fetch_pypi_latest(package: str = "stoa-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a STOA update is available.
    ...
    cache_file = stoa_home / ".update_check"
    embedded_rev = os.environ.get("STOA_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $STOA_HOME/stoa-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = stoa_home / "stoa-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active STOA git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$STOA_HOME/stoa-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
_RELEASE_URL_BASE = "https://github.com/STOAGENT/stoa-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    STOA checkout. Cached per-process. Release URL always points at the
    canonical STOAGENT/stoa-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"STOA Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else STOA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = STOA_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from stoa_cli.codex_runtime_switch import get_current_runtime
        from stoa_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from stoa_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built stoa; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else STOA_AGENT_LOGO
        console.print(_logo)
        console.print()
