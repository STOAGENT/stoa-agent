"""stoa hooks — inspect and manage shell-script hooks.

Usage::

    stoa hooks list
    stoa hooks test <event> [--for-tool X] [--payload-file F]
    stoa hooks revoke <command>
    stoa hooks doctor

Consent records live under ``~/.stoa/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.stoa/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``stoa hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: stoa hooks {list|test|revoke|doctor}")
        print("Run 'stoa hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.stoa/config.yaml.")
        print("See `stoa hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `stoa hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `stoa hooks test` and `stoa hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (STOA wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `stoa hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `stoa hooks doctor` would execute
