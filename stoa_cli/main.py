...
Usage:
    ...
    hermes uninstall           Uninstall STOA Agent
...
def _detect_concurrent_stoa_instances(
    ...
    shims (``hermes.exe`` / ``stoa-gateway.exe``) and returns ``(pid,
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hermes.exe`` / ``stoa-gateway.exe`` shims
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/stoa-agent).
...
def _run_pre_update_backup(args) -> None:
    ...
    STOA_HOME directories.  The ``--backup`` flag on ``hermes update``
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("stoa_requires"):
            print(f"Requires:     Hermes {data['stoa_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.stoa_requires:
        print(f"  Requires: Hermes {mf.stoa_requires}")
...
def main():
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for STOA Agent. Use 'hermes debug share' to "
