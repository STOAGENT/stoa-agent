...
Pure logic: walks a STOA config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `stoa doctor` and a future `stoa migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a STOA config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a STOA config and return retirement issues.
