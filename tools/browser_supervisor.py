...
One ``CDPSupervisor`` runs per STOA ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "stoa-dialog-bridge.invalid"
...
(() => {
  if (window.__stoaDialogBridgeInstalled) return;
  window.__stoaDialogBridgeInstalled = true;
  const ENDPOINT = "http://stoa-dialog-bridge.invalid/";
