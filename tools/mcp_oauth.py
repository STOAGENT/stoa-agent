...
This module provides the glue:
    - ``STOATokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "STOA Agent"
"""
...
from stoa_constants import secure_parent_dir
...
def _get_token_dir() -> Path:
    ...
    Uses STOA_HOME so each profile gets its own OAuth tokens.
    Layout: ``STOA_HOME/mcp-tokens/``
    """
    try:
...
# ---------------------------------------------------------------------------
# STOATokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class STOATokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        STOA_HOME/mcp-tokens/<server_name>.json         -- tokens
        STOA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        STOA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # STOA records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to STOA.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "STOA Agent")
    scope = cfg.get("scope")
