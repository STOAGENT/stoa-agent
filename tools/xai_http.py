...
def has_xai_credentials() -> bool:
    ...
    2. ``~/.stoa/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_stoa_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.stoa/.env`` first, then ``os.environ``.

    Wraps :func:`stoa_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from stoa_cli.config import get_env_value as _stoa_get_env_value

        value = _stoa_get_env_value(name)
...
def stoa_xai_user_agent() -> str:
    ...
    try:
        ...
        __version__ = "unknown"
    return f"Stoa-Agent/{__version__}"
