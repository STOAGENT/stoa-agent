...
_STOA_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter STOA-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_STOA_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _STOA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_STOA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_STOA_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _STOA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_stoa_home(sanitized)
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("STOA_GIT_BASH_PATH")
    ...
    if _stoa_portable_git:
        for candidate in (
            ...
            os.path.join(_stoa_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. STOA Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set STOA_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_STOA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_STOA_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _STOA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    _inject_context_stoa_home(run_env)

    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,
    # npm …) into {STOA_HOME}/home/ when that directory exists.  Only the
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``STOA_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under STOA_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_stoa_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "stoa_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._stoa_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_stoa_pgid", None)
                    if pgid is None:
                        raise
