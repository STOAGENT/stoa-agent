...
def _is_stoa_provider_credential(name: str) -> bool:
    """True if ``name`` is a STOA-managed provider credential (API key,
    token, or similar) per ``_STOA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-STOA API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _STOA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are STOA-managed provider credentials (from
    ``_STOA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a STOA-managed
    ...
    Non-STOA third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_stoa_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register STOA provider "
                "credential %r (blocked by _STOA_PROVIDER_ENV_BLOCKLIST). "
