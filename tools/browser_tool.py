...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    stoa_node_bin = str(stoa_home / "node" / "bin")
    stoa_node_root = str(stoa_home / "node")
    stoa_nm_bin = str(stoa_home / "node_modules" / ".bin")
    return [stoa_node_bin, stoa_node_root, stoa_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.stoa/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.stoa/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when STOA silently
...
def _run_chrome_fallback_command(
    ...
    if not _chromium_installed():
        if _running_in_docker():
            hint = (
                ...
                "docker pull ghcr.io/nousresearch/stoa-agent:latest"
            )
        else:
            hint = (
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the STOA CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-stoa_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed stoa processes — this way every clean stoa exit sweeps
    ...
    # Sweep orphans from other crashed stoa processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live stoa processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current stoa PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live stoa process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning stoa process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced stoa PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent stoa instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-stoa_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live stoa process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, STOA-managed
    ...
    # Build an extended search PATH including STOA-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                stoa_nm = str(get_stoa_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=stoa_nm)
            if not recheck:
                stoa_node_bin = str(get_stoa_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=stoa_node_bin)
            if not recheck:
                stoa_node_root = str(get_stoa_home() / "node")
                recheck = shutil.which("agent-browser", path=stoa_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this stoa PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = stoa_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    from stoa_constants import get_stoa_dir
    screenshots_dir = get_stoa_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/stoa-agent:latest")
                else:
