...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("STOA_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("STOA_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("STOA_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set STOA_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind STOA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds STOA_SESSION_PLATFORM via
    ...
    if env_var_enabled("STOA_CRON_SESSION"):
        return False
    if env_var_enabled("STOA_GATEWAY_SESSION"):
        return True
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bstoa\s+gateway\s+(stop|restart)\b', "stop/restart stoa gateway (kills running agents)"),
    (r'\bstoa\s+update\b', "stoa update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart stoa-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart stoa-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(stoa|gateway|cli\.py)\b', "kill stoa/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill stoa` but not
    # `kill -9 $(pgrep -f stoa)` because the substitution is opaque
...
# Grammar-anchored single-token form. Canonical well-formed shape:
#
#     ^ APPROVE
#     ^ (APPROVE)
#     ^ ( APPROVE )
#
# (anchored at line start, optional parentheses, optional surrounding
# whitespace, the literal verdict token, nothing else). This is the
# 'safe' grammar the smart-approve parser locks against — every other
# shape falls through to the escalate / deny fallbacks above.
_SMART_APPROVE_CANONICAL_RE = re.compile(
    r"^\s*\(?\s*APPROVE\s*\)?\s*$"
)
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("STOA_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    if is_gateway or env_var_enabled("STOA_EXEC_ASK"):
