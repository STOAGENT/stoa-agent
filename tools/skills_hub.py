...
Used by stoa_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by STOA) but not activated
...
def _load_stoa_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for STOA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if STOA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - STOA_INDEX_CACHE_FILE.stat().st_mtime
            if age < STOA_INDEX_TTL:
                return json.loads(STOA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    # Cache locally
    try:
        STOA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        STOA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
