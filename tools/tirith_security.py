...
it is automatically downloaded from GitHub releases to $STOA_HOME/bin/tirith.
...
def _get_stoa_home() -> str:
    """Return the Hermes home directory, respecting STOA_HOME env var."""
    return str(get_stoa_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_stoa_home(), ".tirith-install-failed")
...
def _stoa_bin_dir() -> str:
    """Return $STOA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_stoa_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $STOA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_stoa_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $STOA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $STOA_HOME/bin/tirith
    ...
    stoa_bin = os.path.join(_stoa_bin_dir(), "tirith")
    if os.path.isfile(stoa_bin) and os.access(stoa_bin, os.X_OK):
        _resolved_path = stoa_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return stoa_bin
