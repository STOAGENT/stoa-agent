# OpenAI-Compatible API Server for STOA Agent
...
SSE streaming. By exposing this endpoint, stoa-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  stoa-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.stoa/.env`
2. Run `stoa gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with stoa-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns stoa-agent as a model) |
...
Authorization: Bearer stoa-api-key-here
...
But stoa-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "stoa-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured stoa-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of stoa-agent's core prompt
...
Once implemented, stoa-agent works as a drop-in backend for:
