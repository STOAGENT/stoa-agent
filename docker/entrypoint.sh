...
if [ "$(id -u)" = "0" ]; then
    ...
    if [ -n "$STOA_UID" ] && [ "$STOA_UID" != "10000" ]; then
        needs_chown=true
    elif [ "$(stat -c %u "$STOA_HOME" 2>/dev/null)" != "$actual_stoa_uid" ]; then
        needs_chown=true
    fi
...
echo "docker" > "${STOA_HOME:=/opt/data}/.install_method" 2>/dev/null || true

# Create essential directory structure.  Cache and platform directories
# (cache/images, cache/audio, platforms/whatsapp, etc.) are created on
# demand by the application — don't pre-create them here so new installs
# get the consolidated layout from get_stoa_dir().
# The "home/" subdirectory is a per-profile HOME for subprocesses (git,
# ssh, gh, npm …).  Without it those tools write to /root which is
# ephemeral and shared across profiles.  See issue #4426.
mkdir -p "$STOA_HOME"/{cron,sessions,logs,hooks,memories,skills,skins,plans,workspace,home}
...
if [ ! -f "$STOA_HOME/SOUL.md" ]; then
    cp "$INSTALL_DIR/docker/SOUL.md" "$STOA_HOME/SOUL.md"
fi
...
if [ ! -f "$STOA_HOME/auth.json" ] && [ -n "$STOA_AUTH_JSON_BOOTSTRAP" ]; then
    printf '%s' "$STOA_AUTH_JSON_BOOTSTRAP" > "$STOA_HOME/auth.json"
    chmod 600 "$STOA_HOME/auth.json"
fi
...
# cleanup is needed.
case "${STOA_DASHBOARD:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        dash_host="${STOA_DASHBOARD_HOST:-0.0.0.0}"
        dash_port="${STOA_DASHBOARD_PORT:-9119}"
