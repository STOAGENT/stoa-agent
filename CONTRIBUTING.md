...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [STOA Discord](https://discord.gg/stoa-xyz). Users can install it with `hermes skills install`.
...
├── toolsets.py               # Tool groupings and presets (stoa-cli, hermes-telegram, etc.)
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `hermes setup` or update `~/.stoa/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "STOA Agent" is the secondary collaborator. If the contributor's commit shows "STOA Agent" as author because they used Hermes to draft the skill, replace it with their actual name — credit the human, not the tool.
