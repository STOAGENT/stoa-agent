...
Let the stoa agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where stoa runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `stoa meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `stoa meet` CLI |
...
| `cli.py` | `stoa meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$STOA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `stoa meet node {run,list,approve,remove,status,ping}` |
...
stoa meet install                                      # pip + Chromium
stoa meet setup                                        # preflight
stoa meet auth                                         # optional
stoa meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
stoa meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.stoa/.env
stoa meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
stoa meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
stoa meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.stoa/.env
stoa meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, stoa will **not** switch your system audio input automatically — the
...
stoa plugins enable google_meet
stoa meet node run --display-name my-mac --host 0.0.0.0 --port 18789
