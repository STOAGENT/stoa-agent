"""`stoa meet node ...` subcommand tree.

Wired into the existing ``stoa meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``stoa meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             stoa meet node approve <name> ws://<host>:{args.port} {token}")
        try:
