...
session-scoped state file under ``$STOA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $STOA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_stoa_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this stoa install, leave it first —
    ...
    env = os.environ.copy()
    env["STOA_MEET_URL"] = url
    env["STOA_MEET_OUT_DIR"] = str(out)
    env["STOA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["STOA_MEET_HEADED"] = "1"
    if auth_state:
        env["STOA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["STOA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if STOA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["STOA_MEET_MODE"] = mode
    if realtime_model:
        env["STOA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["STOA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["STOA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["STOA_MEET_REALTIME_KEY"] = realtime_api_key
