"""disk_cleanup — ephemeral file cleanup for STOA Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under STOA_HOME)
...
try:
    ...
    def get_stoa_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("STOA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".stoa").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$STOA_HOME/logs/``."""
    return get_stoa_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$STOA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    ...
    try:
        path.resolve().relative_to(stoa_home)
        return True
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside STOA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under STOA_HOME (but leave STOA_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "stoa-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(stoa_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == stoa_home:
                continue
            try:
                rel_parts = dirpath.relative_to(stoa_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    stoa_home = get_stoa_home()
    try:
        rel = path.resolve().relative_to(stoa_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "stoa-agent",
        }:
            return None
