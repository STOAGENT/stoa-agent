"""
Microsoft Teams platform adapter for STOA Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "STOA"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        stoa_action = data.get("stoa_action", "")
        session_key = data.get("session_key", "")

        if not stoa_action or not session_key:
        ...
        choice = choice_map.get(stoa_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="stoa_approve",
                    data={**btn_data_base, "stoa_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="stoa_approve",
                    data={**btn_data_base, "stoa_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="stoa_approve",
                    data={**btn_data_base, "stoa_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="stoa_approve",
                    data={**btn_data_base, "stoa_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"STOA\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.stoa/.env")
