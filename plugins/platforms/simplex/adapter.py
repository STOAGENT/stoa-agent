"""SimpleX Chat platform adapter (STOA plugin).
...
This adapter ships as a STOA platform plugin under
``plugins/platforms/simplex/``. The STOA plugin loader scans the
...
discoverable and `stoa setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside STOA — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"stoa-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``stoa setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.stoa/.env`` via ``stoa_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("stoa_cli.config not available; set SIMPLEX_* vars manually in ~/.stoa/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `stoa gateway status` without
