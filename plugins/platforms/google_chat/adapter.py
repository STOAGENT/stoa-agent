...
# ``model_tools`` import time, so ``stoa status``, ``stoa chat``, etc.
...
# ``stoa_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "STOA is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a stoa MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${STOA_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "STOA is thinking…".
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${STOA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a stoa MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'STOA is thinking…' marker message.
        ...
        "STOA is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "STOA is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "STOA is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "STOA is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "STOA is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``stoa setup``.

    The setup wizard at ``stoa_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.stoa/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. stoa-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.stoa/.env")
    print_info("Restart the gateway: stoa gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``stoa cron`` running as a
    separate process from ``stoa gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the STOA plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'stoa-agent[google_chat]'",
        ...
        # the "STOA is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'STOA is thinking…' marker message "
