"""
LINE Messaging API platform adapter for STOA Agent.
...
to STOA Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``STOA_HOME``. PR #8398.
        """
        from aiohttp import web
...
def is_connected(config) -> bool:
    """Surface in ``stoa status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``stoa status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``stoa setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.stoa/.env`` via ``stoa_cli.config``.
    """
    print()
    ...
    try:
        from stoa_cli.config import get_env_var, set_env_var
    except ImportError:
        print("stoa_cli.config not available; set LINE_* vars manually in ~/.stoa/.env")
        return
