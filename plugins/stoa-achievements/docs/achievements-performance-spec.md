# STOA Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: stoa-achievements plugin
...
### `GET /api/plugins/stoa-achievements/achievements`
...
### `POST /api/plugins/stoa-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/stoa-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/stoa-achievements/overview`
...
- Persist plugin data under:
  - `~/.stoa/plugins/stoa-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.stoa/plugins/stoa-achievements/scan_checkpoint.json`
...
- `~/.stoa/plugins/stoa-achievements/`
