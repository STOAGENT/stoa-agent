(function () {
  "use strict";
  // stoa-achievements dashboard plugin
  // Originally authored by @PCinkusz — https://github.com/PCinkusz/stoa-achievements (MIT).
  // Bundled into stoa-agent. Upstream repo remains the staging ground for new
  ...
  if (!SDK || !window.__STOA_PLUGINS__) return;
  ...
  async function api(path, options) {
    const url = "/api/plugins/stoa-achievements" + path;
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("HERMES AGENT  ·  stoa-agent.nousresearch.com", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function tweetText() {
      ...
      const tmpl = tx(t, "share.tweet_text", "Just unlocked {tier_part}\"{name}\" in STOA Agent ☤", {
        tier_part: tierPart,
        name: achievement.name,
      });
      return tmpl + "\n\n@stoa-xyz · https://stoa-agent.nousresearch.com";
    }
  ...
  window.__STOA_PLUGINS__.register("stoa-achievements", AchievementsPage);
})();
