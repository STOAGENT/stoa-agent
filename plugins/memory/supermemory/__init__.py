...
def _load_supermemory_config(stoa_home: str) -> dict:
    config = _default_config()
    config_path = Path(stoa_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, stoa_home: str) -> None:
    config_path = Path(stoa_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `stoa memory setup`.
        # All other options are documented for $STOA_HOME/supermemory.json
    ...
    def save_config(self, values, stoa_home):
        ...
        _save_supermemory_config(sanitized, stoa_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from stoa_constants import get_stoa_home
        self._stoa_home = kwargs.get("stoa_home") or str(get_stoa_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._stoa_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "stoa", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "stoa_memory", "target": target, "type": "explicit_memory"},
