...
Handles: stoa honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "stoa" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all STOA profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'stoa honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'stoa honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `stoa update` -- no output, no exceptions.
    """
    try:
        from stoa_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current STOA profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $STOA_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_stoa_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives STOA persistent cross-session memory.")
    ...
    stoa_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'stoa honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = stoa_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        stoa_host["peerName"] = new_peer

    current_ai = stoa_host.get("aiPeer") or cfg.get("aiPeer", "stoa")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        stoa_host["aiPeer"] = new_ai

    current_workspace = stoa_host.get("workspace") or cfg.get("workspace", "stoa")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        stoa_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = stoa_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        stoa_host["observationMode"] = new_obs
    else:
        stoa_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(stoa_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        stoa_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        stoa_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = stoa_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        stoa_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = stoa_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        stoa_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                stoa_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(stoa_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            stoa_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        stoa_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        stoa_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        stoa_host["dialecticReasoningLevel"] = new_reasoning
    else:
        stoa_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = stoa_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        stoa_host["sessionStrategy"] = new_strat

    stoa_host["enabled"] = True
    stoa_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        stoa_config = load_config()
        stoa_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(stoa_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: stoa config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    stoa honcho status     -- show full config")
    print("    stoa honcho mode       -- change recall/observation mode")
    print("    stoa honcho tokens     -- tune context and dialectic budgets")
    print("    stoa honcho peer       -- update peer names")
    print("    stoa honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active STOA profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: stoa honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'stoa honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: stoa honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        stoa = hosts.get(_host_key(), {})
        user = stoa.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = stoa.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = stoa.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = stoa.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    STOA' identity in Honcho. Seed with 'stoa honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: stoa honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: stoa honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = stoa.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = stoa.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = stoa.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. STOA asks Honcho's AI peer a question")
        ...
        print("\n  Set with: stoa honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'stoa honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'stoa honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    stoa honcho identity --show        — show both peer representations")
        print("    stoa honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → STOA + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → STOA\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives STOA persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  stoa honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'stoa honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'stoa honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  stoa honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'stoa'")
        print("  with Honcho configured and no prior session history.")
        print("  (STOA calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    stoa honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'stoa honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In STOA, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'stoa honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    stoa honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  stoa honcho identity <path/to/SOUL.md>")
    ...
    print("    STOA:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    STOA:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    STOA:   Honcho observes every message and updates representations")
    ...
    print("    STOA:   per-session by default — each run gets its own session")
    print("              Map a custom name:  stoa honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. stoa honcho setup              — configure API key (required)")
        print("  2. stoa honcho migrate            — re-run this walkthrough")
    else:
        print("  1. stoa honcho status             — verify Honcho connection")
        print("  2. stoa                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. stoa honcho identity --show    — verify AI peer representation")
        print("  4. stoa honcho tokens             — tune context and dialectic budgets")
        print("  5. stoa honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'stoa memory setup'...\n")
        from stoa_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``stoa honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``stoa honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to stoa memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to STOA Honcho",
    )
