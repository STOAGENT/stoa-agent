...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/stoa>
...
```bash
stoa honcho setup    # full interactive wizard (cloud or local)
stoa memory setup    # generic picker, also works
```

Or manually:
```bash
stoa config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$STOA_HOME/honcho.json` | Profile-local (isolated STOA instances) |
| 2 | `~/.stoa/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active STOA profile: `stoa` (default) or `stoa.<profile>`.
...
| 4 | `per-session` strategy | STOA session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`stoa-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`stoa`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-stoa-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening stoa in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — STOA session ID (timestamp + hex). Every `stoa` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple STOA profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`stoa`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active STOA profile: `stoa` (default) or `stoa.<profile>` (e.g. `stoa -p coder` → host key `stoa.coder`).
...
| `STOA_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `stoa honcho setup` | Full interactive setup wizard |
| `stoa honcho status` | Show resolved config for active profile |
| `stoa honcho enable` / `disable` | Toggle Honcho for active profile |
| `stoa honcho mode <mode>` | Change recall or observation mode |
| `stoa honcho peer --user <name>` | Update user peer name |
| `stoa honcho peer --ai <name>` | Update AI peer name |
| `stoa honcho tokens --context <N>` | Set context token budget |
| `stoa honcho tokens --dialectic <N>` | Set dialectic max chars |
| `stoa honcho map <name>` | Map current directory to a session name |
| `stoa honcho sync` | Create host blocks for all STOA profiles |
