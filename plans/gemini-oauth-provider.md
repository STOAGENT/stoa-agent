...
## Client ID
- Need to register a "Desktop app" OAuth client on a STOA GCP project
...
## Token Lifecycle
- Store at `~/.stoa/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `stoa_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `stoa_cli/models.py` — Add Gemini model catalog
4. `stoa_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `stoa_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `stoa_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- STOA GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via STOA_GEMINI_CLIENT_ID env var
...
- stoa-agent Copilot OAuth: `stoa_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
