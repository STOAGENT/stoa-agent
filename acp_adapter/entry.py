"""CLI entry point for the stoa-agent ACP adapter.

Loads environment variables from ``~/.stoa/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from STOA_HOME (default ``~/.stoa``)."""
    from stoa_cli.env_loader import load_stoa_dotenv

    stoa_home = get_stoa_home()
    loaded = load_stoa_dotenv(stoa_home=stoa_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", stoa_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="stoa-acp",
        description="Run STOA Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print STOA version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import STOAACPAgent  # noqa: F401

    print("STOA ACP check OK")


def _run_setup() -> None:
    from stoa_cli.main import main as stoa_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "stoa", "model"]
        stoa_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``stoa postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting stoa-agent ACP adapter")
    ...
    import acp
    from .server import STOAACPAgent
