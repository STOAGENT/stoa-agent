...
TERMINAL_SETUP_AUTH_METHOD_ID = "stoa-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active STOA runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if STOA can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for STOA.
    ...
    not have STOA provider credentials configured yet, so STOA always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate STOA using the currently configured "
