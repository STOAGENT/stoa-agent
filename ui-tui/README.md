...
React + Ink terminal UI for STOA. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `STOA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.stoa/.stoa_history` or under `STOA_HOME`.
...
```text
ui-tui/
  packages/stoa-ink/   forked Ink renderer (local dep)
  src/
...
tui_gateway/
  ...
  slash_worker.py        persistent STOACLI subprocess for slash commands
```
