...
// Mirrors @stoa/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @stoa/ink is bundled separately from packages/.
...
// Terminal. Explicit STOA_TUI_THEME / COLORFGBG signals above still win,
...
// `STOA_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `STOA_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `STOA_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `STOA_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default STOA palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.STOA_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.STOA_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.STOA_TUI_BACKGROUND ?? '')
