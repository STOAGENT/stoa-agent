...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.STOA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep STOA on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via STOA_TUI_TRUECOLOR=1.
