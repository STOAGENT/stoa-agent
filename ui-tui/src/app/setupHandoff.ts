import type { RunExternalProcess } from '@stoa/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`stoa ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching stoa: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`stoa ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
