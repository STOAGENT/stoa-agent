...
> ⚠️ **Authorization required**: this skill bypasses LLM safety filters on
> API-served models (OpenAI, Anthropic, xAI, Google). Using it against
> APIs you don't own or don't have explicit authorization to test
> violates their Terms of Service.
>
> STOA Agent ships this skill in `optional-skills/red-teaming/` (NOT
> the default `skills/` directory). To load it, set
> `STOA_ENABLE_REDTEAM=1` in your environment. By enabling, you confirm
> you have authorization for the systems you're testing.
...
12. **execute_code sandbox has no env vars** — When Hermes runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.stoa/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.stoa/.env"))`
