...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("STOA_HOME", os.path.expanduser("~/.stoa")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
MODEL_STRATEGIES = {
    ...
    # Nous/STOA models — already uncensored, just needs clean prompt
    "stoa": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "stoa" in model_lower or "nous" in model_lower:
        return "stoa"
...
def _get_current_model() -> tuple:
    """Read current model and provider from STOA config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.stoa/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart STOA for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart STOA for changes to take effect.")
