...
Usage (via cron with --no-agent):
    ...
    stoa cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$STOA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "STOA-Watcher/1.0"})
