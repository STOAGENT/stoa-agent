...
# become ``~/.stoa`` — the real Hermes home — not the broken ``~/.Hermes``).
...
def _case_preserving_replacement(replacement: str):
    ...
    ``~/.stoa/config.yaml`` (the real Hermes home) instead of the broken
    ``~/.Hermes/config.yaml``.
    """
...
class Migrator:
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            stoa_cfg_path = self.target_root / "config.yaml"
            stoa_cfg = load_yaml_file(stoa_cfg_path)
            discord_hermes = stoa_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                stoa_cfg["discord"] = discord_hermes
                dump_yaml_file(stoa_cfg_path, stoa_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        stoa_cfg_path = self.target_root / "config.yaml"
        stoa_cfg = load_yaml_file(stoa_cfg_path)
        browser_hermes = stoa_cfg.get("browser") or {}
        changed = False
        ...
        if changed:
            stoa_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(stoa_cfg_path)
                dump_yaml_file(stoa_cfg_path, stoa_cfg)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("STOA_HOME") or str(Path.home() / ".hermes"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
