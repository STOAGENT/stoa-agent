#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `stoa kanban list` and
...
def stoa_available() -> bool:
    return shutil.which("stoa") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed stoa CLI."""
    try:
        out = subprocess.run(
            ["stoa", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `stoa kanban list`
    out = subprocess.run(
        ["stoa", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not stoa_available():
        print("ERROR: 'stoa' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
