...
Maps each role archetype to the STOA skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public stoa-agent
...
## STOA skills relevant to video production

### Visual / rendering skills (`stoa-agent/skills/creative/`)
...
### Design / pre-production skills (`stoa-agent/skills/creative/`)
...
### Audio / media skills (`stoa-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`stoa-agent/skills/devops/`)
...
These are **not** STOA skills but external CLIs / APIs that profiles invoke.
...
## Built-in STOA tools for media review

These are native STOA tools — not invoked via terminal but through their own
...
key is present in `~/.stoa/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every STOA profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.stoa/.env` → environment variable.
...
When a new STOA-public video skill ships:
