...
stoa kanban watch --tenant <project-slug>

# Snapshot of the board
stoa kanban list --tenant <project-slug>
stoa kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
stoa kanban stats --tenant <project-slug>

# Visual dashboard (browser)
stoa dashboard

# Inspect a specific task (includes comments + events)
stoa kanban show <task-id>

# Follow a single task's event stream
stoa kanban tail <task-id>
```

Verify available subcommands with `stoa kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `stoa kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `stoa kanban block <id>` then `stoa kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `stoa kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `stoa kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `stoa profile list` (profile exists?), `stoa status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(stoa kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
stoa kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
stoa kanban block <task-id>
stoa kanban archive <task-id>

# Diagnose what happened
stoa kanban show <task-id>      # task body, comments, recent events
stoa kanban tail <task-id>      # follow the live event stream
stoa kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `stoa kanban create` for the director
...
while true; do
    clear
    stoa kanban list --tenant <slug>
    echo "---"
    stoa kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `stoa kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `stoa profile list` and start
