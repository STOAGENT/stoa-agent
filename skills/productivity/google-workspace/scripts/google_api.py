#!/usr/bin/env python3
"""Google Workspace API CLI for STOA Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing STOA-facing JSON contract and falls back to the Python client
...
from _stoa_home import get_stoa_home
...
def _gws_binary() -> str | None:
    override = os.getenv("STOA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for STOA Agent")
