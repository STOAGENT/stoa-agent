...
## 5. STOA Configuration

### Config location

`$STOA_HOME/config.yaml` (defaults to `~/.stoa/config.yaml` when `STOA_HOME` is unset)
...
Restart the STOA session for changes to take effect. The MCP connection is
