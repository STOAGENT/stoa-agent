...
# 5. Symlinks the 'stoa' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ STOA Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/stoa-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink stoa into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up stoa command..."

STOA_BIN="$SCRIPT_DIR/venv/bin/stoa"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$STOA_BIN" "$COMMAND_LINK_DIR/stoa"
echo -e "${GREEN}✓${NC} Symlinked stoa → $COMMAND_LINK_DISPLAY_DIR/stoa"
...
echo ""
echo "Syncing bundled skills to ~/.stoa/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$STOA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  stoa status        # Check configuration"
if is_termux; then
    echo "  stoa gateway       # Run gateway in foreground"
else
    echo "  stoa gateway install # Install gateway service (messaging + cron)"
fi
echo "  stoa cron list     # View scheduled jobs"
echo "  stoa doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m stoa_cli.main setup
fi
