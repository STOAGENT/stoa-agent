...
export class GatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    const resolved = token ?? window.__STOA_SESSION_TOKEN__ ?? "";
    if (!resolved) {
      this.setState("error");
      throw new Error(
        "Session token not available — page must be served by the STOA dashboard",
      );
    }

    const scheme = location.protocol === "https:" ? "wss:" : "ws:";
    const ws = new WebSocket(
      `${scheme}//${location.host}${STOA_BASE_PATH}/api/ws?token=${encodeURIComponent(resolved)}`,
    );
    this.ws = ws;
