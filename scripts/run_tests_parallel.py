...
Environment:
    STOA_TEST_WORKERS  Override worker count (default: os.cpu_count())
    STOA_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
# via --file-timeout or STOA_TEST_FILE_TIMEOUT.
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/stoa-agent/stoa-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("STOA_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $STOA_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("STOA_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("STOA_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            "Default: 600 (10 min), env: STOA_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: STOA_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "paths_positional",
    ...
    # Parse --slice (or STOA_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("STOA_TEST_SLICE")
