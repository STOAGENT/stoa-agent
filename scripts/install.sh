#!/bin/bash
# ============================================================================
# STOA Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/STOAGENT/stoa-agent/v0.14.10/scripts/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. STOA terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:STOAGENT/stoa-agent.git"
REPO_URL_HTTPS="https://github.com/STOAGENT/stoa-agent.git"
STOA_HOME="${STOA_HOME:-$HOME/.stoa}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${STOA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$STOA_INSTALL_DIR"
...
#   code at /usr/local/lib/stoa-agent, command at /usr/local/bin/stoa,
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --stoa-home PATH  Data directory (default: ~/.stoa, or \$STOA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, STOA installs the code under"
            echo "  /usr/local/lib/stoa-agent and links the command into"
            echo "  /usr/local/bin/stoa (FHS layout — matches Claude Code / Codex CLI)."
            ...
            echo "                   Installs optional deps + runs stoa setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ STOA Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by STOA.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `stoa` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $STOA_HOME/stoa-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $STOA_HOME/stoa-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/stoa-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $STOA_HOME/stoa-agent — then preserve it)
#
# Always no-op when the user set --dir or $STOA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in STOA_HOME.
    if is_termux; then
        INSTALL_DIR="$STOA_HOME/stoa-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$STOA_HOME/stoa-agent/.git" ]; then
            INSTALL_DIR="$STOA_HOME/stoa-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/stoa-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/stoa-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/stoa"
        log_info "  Data:    $STOA_HOME (unchanged)"
        return 0
    fi
...
get_stoa_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/stoa" ]; then
        echo "$link_dir/stoa"
    else
        echo "stoa"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/STOAGENT/stoa-agent/v0.14.10/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/stoa-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/stoa-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$STOA_HOME/node/bin/node" ]; then
        export PATH="$STOA_HOME/node/bin:$PATH"
        local found_ver=$("$STOA_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (STOA-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    # Place into ~/.stoa/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$STOA_HOME/node"
    mkdir -p "$STOA_HOME"
    mv "$extracted_dir" "$STOA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$STOA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$STOA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$STOA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. STOA install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "STOA Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="stoa-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if STOA behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up stoa command..."

    if [ "$USE_VENV" = true ]; then
        STOA_BIN="$INSTALL_DIR/venv/bin/stoa"
    else
        STOA_BIN="$(which stoa 2>/dev/null || echo "")"
        if [ -z "$STOA_BIN" ]; then
            log_warn "stoa not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$STOA_BIN" ]; then
        log_warn "stoa entry point not found at $STOA_BIN"
    ...
    # Create a user-facing shim for the stoa command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $STOA_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$STOA_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/stoa"
    cat > "$command_link_dir/stoa" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$STOA_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/stoa"
    log_success "Installed stoa launcher → $command_link_display_dir/stoa"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "stoa command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v stoa' \
                >/dev/null 2>&1; then
        ...
        log_info "stoa not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# STOA Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# STOA Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# STOA Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so stoa works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.stoa directory structure (config at top level, code in subdir)
    mkdir -p "$STOA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.stoa/.env (top level, easy to find)
    if [ ! -f "$STOA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$STOA_HOME/.env"
            log_success "Created ~/.stoa/.env from template"
        else
            touch "$STOA_HOME/.env"
            log_success "Created ~/.stoa/.env"
        fi
    else
        log_info "~/.stoa/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$STOA_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.stoa/config.yaml (top level, easy to find)
    if [ ! -f "$STOA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$STOA_HOME/config.yaml"
            log_success "Created ~/.stoa/config.yaml from template"
        fi
    else
        log_info "~/.stoa/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$STOA_HOME/SOUL.md" ]; then
        cat > "$STOA_HOME/SOUL.md" << 'SOUL_EOF'
# STOA Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how STOA communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.stoa/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.stoa/"

    # Seed bundled skills into ~/.stoa/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.stoa/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.stoa/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$STOA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$STOA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.stoa/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# STOA Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; STOA will use the system browser."
        else
            case "$DISTRO" in
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'stoa setup' after install."
        return 0
    fi
    ...
    # Run stoa setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m stoa_cli.main setup < /dev/tty
    else
        python -m stoa_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for STOA to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$STOA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'stoa whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                STOA_CMD="$(get_stoa_command_path)"
                $STOA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'stoa whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'stoa gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $STOA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $STOA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: stoa gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: stoa gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $STOA_CMD gateway > "$STOA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.stoa/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: stoa gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: stoa gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $STOA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $STOA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $STOA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}stoa${NC}              Start chatting"
    echo -e "   ${GREEN}stoa setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}stoa config${NC}       View/edit configuration"
    echo -e "   ${GREEN}stoa config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}stoa gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}stoa update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'stoa' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'stoa' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'stoa' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$STOA_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$STOA_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$STOA_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$STOA_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$STOA_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up STOA for pip install"
    ...
    STOA_CMD="$(command -v stoa 2>/dev/null || echo "")"
    if [ -n "$STOA_CMD" ]; then
        log_info "Running stoa setup..."
        "$STOA_CMD" setup
    else
        log_warn "stoa command not found on PATH"
        log_info "Try: python -m stoa_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$STOA_HOME/.install_method"
}
