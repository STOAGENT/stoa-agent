 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.stoa/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.stoa', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.stoa', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.stoa', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.stoa', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *STOA Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['STOA Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore STOA' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `stoa_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            // Gap-audit 2026-06-01 (JS-WA-01): argv form via execFileSync — no
            // shell, so a filePath like /tmp/$(cmd).mp3 cannot inject commands
            // (execSync used /bin/sh -c and JSON.stringify does NOT escape $/`).
            execFileSync(
              'ffmpeg',
