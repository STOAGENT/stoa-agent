...
VERSION_FILE = REPO_ROOT / "stoa_cli" / "__init__.py"
...
AUTHOR_MAP = {
    # STOA project identity (commits from infra/build/release automation)
    "stoa@stoax.xyz": "stoaxyz",
    ...
    "274096618+stoa-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "stoa@nousresearch.com": "stoa-xyz",
    ...
    "stoa@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (STOA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (STOA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: stoa-eval community link)
    ...
    "stoaagent26@gmail.com": "stoaagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+stoaagent26@users.noreply.github.com": "stoaagent26",
    ...
    "vigo@stoa": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"stoa-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "stoa@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/STOAGENT/stoa-agent",
    ...
    lines.append(f"# STOA Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for STOA Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="STOA Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  STOA Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"STOA Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"STOA Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'STOA Agent v{new_version} ({calver_date})' "
