...
# Bootstrap Open WebUI against STOA Agent's OpenAI-compatible API server.
#
...
#   STOA_API_PORT=8642
#   STOA_API_HOST=127.0.0.1
#   STOA_API_MODEL_NAME='STOA Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-STOA Agent WebUI}"
...
STOA_ENV_FILE="${STOA_ENV_FILE:-$HOME/.stoa/.env}"
STOA_API_PORT="${STOA_API_PORT:-8642}"
STOA_API_HOST="${STOA_API_HOST:-127.0.0.1}"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$STOA_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$STOA_ENV_FILE" 2>/dev/null || true
}
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to STOA Agent
...
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.stoa/logs/openwebui.log
StandardError=append:%h/.stoa/logs/openwebui.error.log
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$STOA_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
  ...
  upsert_env API_SERVER_MODEL_NAME "$STOA_API_MODEL_NAME" "$STOA_ENV_FILE"
