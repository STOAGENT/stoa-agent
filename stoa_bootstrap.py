"""Windows UTF-8 bootstrap for STOA entry points.
...
should be imported at the very top of every STOA entry point
(``hermes``, ``stoa-agent``, ``hermes-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import stoa_bootstrap``
# (or ``from stoa_bootstrap import apply_windows_utf8_bootstrap``) at
