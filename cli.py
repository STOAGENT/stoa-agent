...
def load_cli_config() -> Dict[str, Any]:
    ...
    If STOA_IGNORE_USER_CONFIG=1 is set (via ``hermes chat --ignore-user-config``),
...
class HermesCLI:
    ...
    def __init__(
        ...
        # by `hermes chat --ignore-rules` in stoa_cli/main.py. When true we
        ...
        # it's shared across all Hermes processes for this STOA_HOME.
    ...
    def show_banner(self):
        ...
        if is_nous_stoa_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  STOA Hermes 3 & 4 models are NOT agentic and are not "
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to stoa_cli.curator so the CLI and the `hermes curator`
