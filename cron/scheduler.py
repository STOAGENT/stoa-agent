...
Uses a file-based lock (~/.stoa/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `stoa update` reloads
# the module) fail with ModuleNotFoundError for stoa_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from stoa_constants import get_stoa_home
from stoa_cli._subprocess_compat import windows_hide_flags
from stoa_cli.config import load_config, _expand_env_vars
from stoa_time import now as _stoa_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``stoa tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from stoa_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_stoa_home: Path | None = None


def _get_stoa_home() -> Path:
    """Resolve STOA home dynamically while preserving test monkeypatch hooks."""
    return _stoa_home or get_stoa_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    stoa_home = _get_stoa_home()
    lock_dir = stoa_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific STOA profile.
    ...
    the scheduler's test/override hook and a context-local STOA home override
    both point at the resolved profile directory so _get_stoa_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_stoa_home() callers agree on the same home.
    ...
    global _stoa_home
    prior_override = _stoa_home
    env_snapshot = os.environ.copy()

    from stoa_cli.profiles import normalize_profile_name, resolve_profile_env
    from stoa_constants import reset_stoa_home_override, set_stoa_home_override
    ...
    override_token = None
    try:
        override_token = set_stoa_home_override(profile_home)
        _stoa_home = profile_home
        logger.info(
            "Job '%s': using STOA profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _stoa_home = prior_override
        if override_token is not None:
            reset_stoa_home_override(override_token)
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("STOA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid STOA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within STOA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against STOA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_stoa_home() / "scripts"
    ...
    # escape — scripts MUST reside within STOA_HOME/scripts/.
    try:
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _stoa_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_stoa_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    # Audit Phase-1A (PROBE-CRIT-010 / F-L05-027): scheduler.run() used
    # to SET this env var but never RESTORE it. When the scheduler was
    # invoked inline (e.g. from a test runner, from another stoa
    # subprocess that imports cron.scheduler, or from a long-lived
    # gateway that schedules a one-off job and continues), the env leak
    # made the rest of the process behave as if it were always a cron
    # session — approval cron_mode applied to interactive prompts. The
    # try/finally below captures + restores the prior value.
    _prev_cron_session = os.environ.get("STOA_CRON_SESSION")
    os.environ["STOA_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed STOA_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads STOA_SESSION_PLATFORM
    #     and STOA_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via STOA_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     STOA_SESSION_PLATFORM == "telegram".
    ...
    #     STOA_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read STOA_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the STOA_CRON_AUTO_DELIVER_* vars set
    # below, so clearing STOA_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "STOA_CRON_AUTO_DELIVER_PLATFORM",
        "STOA_CRON_AUTO_DELIVER_CHAT_ID",
        "STOA_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_stoa_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_stoa_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["STOA_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["STOA_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["STOA_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("STOA_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_stoa_home() / "config.yaml")
        ...
        prefill_file = os.getenv("STOA_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_stoa_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject STOA_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # STOA_HOME. When a workdir is configured, also inject project
        ...
        # override via STOA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("STOA_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid STOA_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_stoa_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _stoa_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _stoa_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set STOA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("STOA_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid STOA_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        # STOA home override, scheduler _stoa_home hook, and temporary
