...
def server():
    with patch.dict(
        "sys.modules",
        {
            "stoa_constants": MagicMock(
                get_stoa_home=MagicMock(return_value="/tmp/stoa_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'stoa-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'stoa-release' patched"
    }
