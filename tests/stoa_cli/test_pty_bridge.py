"""Unit tests for stoa_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf stoa-ok"])
        try:
            output = _read_until(bridge, b"stoa-ok")
            assert b"stoa-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$STOA_PTY_TEST\""],
            env={**os.environ, "STOA_PTY_TEST": "pty-env-works"},
        )
        try:
