"""Tests for IRC gateway configuration via `stoa setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh STOA install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new STOA install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#stoa")
            monkeypatch.setenv("IRC_NICKNAME", "stoa-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `stoa setup gateway` experience with IRC present."""
