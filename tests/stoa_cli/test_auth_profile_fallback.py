...
When ``STOA_HOME`` points to a named profile, ``read_credential_pool()``
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (STOA_HOME == global root), no fallback path runs.
    ...
    stoa_home = tmp_path / "classic"
    stoa_home.mkdir()
    monkeypatch.setenv("STOA_HOME", str(stoa_home))

    _write(stoa_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from stoa_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: STOA_HOME is set to a custom path that is NOT under
    # ~/.stoa/profiles/ — get_default_stoa_root() returns STOA_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from stoa_cli.auth import read_credential_pool, write_credential_pool
