...
from stoa_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    MigrationReport,
    _build_stoa_tools_mcp_entry,
    _format_toml_value,
...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_stoa_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from stoa_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from stoa_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_stoa_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_stoa_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_stoa_tools=False)
        assert called["yes"] is False
    ...
    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_stoa_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(stoa_cfg, codex_home=tmp_path, expose_stoa_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_stoa_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_stoa_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_stoa_tools=False)
        new_text = target.read_text()
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_stoa_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_stoa_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "stoa_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_stoa_tools=False)
        new_text = target.read_text()
...
class TestHermesHomeLeakGuard:
    ...
    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when STOA_HOME points at a pytest
        tempdir, _build_stoa_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "STOA_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/stoa_test",
        )
        entry = _build_stoa_tools_mcp_entry()
        env = entry.get("env", {})
        assert "STOA_HOME" not in env, (
            f"pytest-tempdir STOA_HOME leaked into codex MCP entry: "
            f"{env.get('STOA_HOME')!r}"
        )

    def test_real_stoa_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate STOA_HOME (not a tempdir path) DOES propagate so the
        ...
        entry = _build_stoa_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("STOA_HOME") == real_path

    def test_unset_stoa_home_omits_env_key(self, monkeypatch):
        """When STOA_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever STOA_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "STOA_HOME" not in env, (
            f"STOA_HOME should not be set when env var is unset, got: "
            f"{env.get('STOA_HOME')!r}"
        )
