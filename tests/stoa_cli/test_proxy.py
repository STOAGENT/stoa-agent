"""Tests for the `stoa proxy` subcommand and its upstream adapters."""
...
from stoa_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from stoa_cli.proxy.adapters.nous_portal import NousPortalAdapter
...
def _write_auth_store(stoa_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic STOA_HOME."""
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # STOA_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "stoa_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("STOA_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="stoa login nous"):
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic STOA_HOME."""
    auth_path = stoa_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("stoa_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
from stoa_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("STOA_HOME", str(tmp_path))
    from stoa_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from stoa_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from stoa_cli.proxy.cli import cmd_proxy_start
