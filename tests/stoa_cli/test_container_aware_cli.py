...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode = stoa_home / ".container-mode"
...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("stoa_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_stoa_dev(container_env, monkeypatch):
    """Returns None when STOA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("STOA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_stoa_dev_zero(container_env, monkeypatch):
    """STOA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("STOA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("stoa_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_stoa_home": lambda: stoa_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("STOA_DEV", None)
            info = get_container_exec_info()
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from stoa_cli.main import _exec_in_container
