"""Tests for the top-level `./stoa` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./stoa` should use `stoa_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "stoa"
    called = []

    fake_main_module = types.ModuleType("stoa_cli.main")

    def fake_main():
        called.append("stoa_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "stoa_cli.main", fake_main_module)
