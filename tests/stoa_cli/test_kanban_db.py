"""Tests for the Kanban DB layer (stoa_cli.kanban_db)."""
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("STOA_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import stoa_cli.kanban_db as _kb
...
def test_scratch_workspace_created_under_stoa_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
class TestSharedBoardPaths:
    ...
    must anchor at the **shared root**, not the active profile's STOA_HOME."""

    def _set_home(self, monkeypatch, tmp_path, stoa_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.stoa/kanban.db,
    ...
    def test_docker_custom_stoa_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_explicit_override_via_stoa_kanban_home(
        self, tmp_path, monkeypatch
    ):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("STOA_HOME", str(profile_home))
        monkeypatch.setenv("STOA_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("STOA_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # Worker switches to the profile STOA_HOME and reads.
        monkeypatch.setenv("STOA_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_stoa_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("STOA_HOME", str(default_home))
        monkeypatch.setenv("STOA_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("STOA_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows STOA_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_stoa_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        ...
        monkeypatch.setenv("STOA_HOME", str(default_home))
        monkeypatch.setenv("STOA_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("STOA_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows STOA_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("STOA_HOME", str(default_home))
        monkeypatch.setenv("STOA_KANBAN_DB", "   ")
        monkeypatch.setenv("STOA_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject STOA_KANBAN_DB
        ...
        env = captured["env"]
        assert env["STOA_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["STOA_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["STOA_KANBAN_TASK"] == "t_dispatch_env"
        assert env["STOA_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see stoa_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("stoa_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="stoa_state"):
            conn = kb.connect()
...
def test_resolve_stoa_argv_honors_stoa_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like STOA_BIN lets service managers pin the executable."""
    import shutil
    import stoa_cli.kanban_db as kb
    ...
    assert kb._resolve_stoa_argv() == [str(shim)]


def test_resolve_stoa_argv_stoa_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare STOA_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import stoa_cli.kanban_db as kb
...
def test_resolve_stoa_argv_stoa_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare STOA_BIN does not accept current-directory shadow executables."""
    import sys
    import stoa_cli.kanban_db as kb
...
def test_resolve_stoa_argv_stoa_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved STOA_BIN batch shim is not used as worker argv[0]."""
    import sys
    import stoa_cli.kanban_db as kb
...
def test_resolve_stoa_argv_stoa_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved STOA_BIN command names do not delegate cwd search to Popen."""
    import sys
    import stoa_cli.kanban_db as kb
...
def test_resolve_stoa_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_stoa_argv()
    assert argv == [sys.executable, "-m", "stoa_cli.main"]


def test_resolve_stoa_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `stoa_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m stoa_cli.main --version`
    ...
    assert "STOA Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="stoa_cli.kanban_db"):
        with kb.connect() as conn:
