"""Tests for the ``stoa send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``stoa_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
def test_load_stoa_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``stoa send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``stoa config set``) lives
    ...
    # Force get_stoa_home() to re-resolve under the patched env.
    from importlib import reload

    import stoa_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_stoa_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_stoa_env_does_not_override_existing(tmp_path, monkeypatch):
