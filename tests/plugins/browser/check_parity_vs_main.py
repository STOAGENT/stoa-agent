...
Run from the PR worktree:
    ...
    cd ~/.stoa/stoa-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.stoa/stoa-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.stoa/stoa-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a stoa-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a stoa-agent checkout"
)
...
# Isolated STOA_HOME for the config write.
