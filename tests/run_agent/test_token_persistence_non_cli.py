...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    stoa_state = ModuleType("stoa_state")
    stoa_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "stoa_state", stoa_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "STOA"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "STOA"
