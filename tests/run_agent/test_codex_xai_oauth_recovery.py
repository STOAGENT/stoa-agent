...
3. Multi-turn replay of ``codex_reasoning_items`` (with
   ...
   reverted shortly after: xAI confirmed they explicitly want STOA to
...
# STOA (https://x.ai/news/grok-stoa), making that hint actively wrong:
...
def test_summarize_api_error_surfaces_xai_entitlement_body_verbatim():
    """xAI's OAuth 403 body must surface as-is, with no STOA-side hint."""
    from run_agent import AIAgent
    ...
    # No stale claim that X Premium is incompatible with STOA.
...
def test_codex_reasoning_replay_includes_encrypted_content_for_xai():
    ...
    want STOA to thread encrypted reasoning back across turns for
...
def test_grok_4_3_context_length_is_1m():
    ...
    STOA' substring-match fallback used to return 256k (from the
