...
- 96vc: ACP didn't set STOA_EXEC_ASK, so `check_all_command_guards`
...
1. Setting STOA_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds STOA_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set STOA_INTERACTIVE so
    ...
    (STOA_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When STOA_INTERACTIVE is set and an approval callback is
        ...
        # With STOA_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with STOA_INTERACTIVE the approval path should consult the "
