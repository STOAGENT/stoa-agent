"""
Tests for mcp_serve — STOA MCP server.
...
def _isolate_stoa_home(tmp_path, monkeypatch):
    """Redirect STOA_HOME to a temp directory."""
    monkeypatch.setenv("STOA_HOME", str(tmp_path))
    try:
        import stoa_constants
        monkeypatch.setattr(stoa_constants, "get_stoa_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking stoa_state schema."""
