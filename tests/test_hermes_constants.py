"""Tests for stoa_constants module."""
...
class TestGetDefaultHermesRoot:
    ...
    def test_no_stoa_home_returns_native(self, tmp_path, monkeypatch):
        """When STOA_HOME is not set, returns ~/.stoa."""
    ...
    def test_stoa_home_is_native(self, tmp_path, monkeypatch):
        ...
        assert get_default_stoa_root() == native
    ...
    def test_stoa_home_is_docker(self, tmp_path, monkeypatch):
        """When STOA_HOME points outside ~/.stoa (Docker), returns STOA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("STOA_HOME", str(docker_home))
        assert get_default_stoa_root() == docker_home
    ...
    def test_stoa_home_is_custom_path(self, tmp_path, monkeypatch):
        ...
        assert get_default_stoa_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (STOA_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("STOA_HOME", str(profile))
        assert get_default_stoa_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(stoa_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(stoa_constants, "_container_detected", True)
        assert is_container() is True
