...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("stoa/stoa-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned stoa/* and pr-* branches."""

    def test_prunes_orphaned_stoa_branch(self, git_repo):
        """stoa/stoa-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "stoa/stoa-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "stoa/stoa-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "stoa/stoa-deadbeef" in result.stdout
        ...
        assert "stoa/stoa-deadbeef" in orphaned
        ...
        assert "stoa/stoa-deadbeef" not in result.stdout
