...
from cli import STOACLI, _rich_text_from_ansi
from stoa_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    def test_default_prompt_fragments_use_default_symbol(self):
        ...
        # The "default" skin's prompt_symbol is "›" (single chevron) post-rebrand.
        # "❯" remains the absolute fallback when no skin is active (cli.py:12127).
    ...
    def test_narrow_terminals_compact_voice_prompt_fragments(self):
        ...
        with patch.object(STOACLI, "_get_tui_terminal_width", return_value=50):
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("stoa_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
