...
def stoa_home(tmp_path, monkeypatch):
    ...
    # Clear any cached stoa_home computation
    import stoa_constants
    if hasattr(stoa_constants, "_stoa_home_cache"):
        stoa_constants._stoa_home_cache = None
    return home
...
def test_save_conversation_writes_under_stoa_home(stoa_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.stoa/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the STOA_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "stoa_constants"]:
