"""Shared fixtures for the stoa-agent test suite.
...
# STOA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_STOA_BEHAVIORAL_VARS = frozenset({
    "STOA_YOLO_MODE",
    "STOA_INTERACTIVE",
    "STOA_QUIET",
    "STOA_TOOL_PROGRESS",
    ...
    "STOA_SESSION_CHAT_ID",
    "STOA_SESSION_CHAT_NAME",
    "STOA_SESSION_THREAD_ID",
    "STOA_SESSION_SOURCE",
    "STOA_SESSION_KEY",
    "STOA_GATEWAY_SESSION",
    "STOA_PLATFORM",
    "STOA_MODEL",
    "STOA_INFERENCE_MODEL",
    "STOA_INFERENCE_PROVIDER",
    "STOA_TUI_PROVIDER",
    "STOA_MANAGED",
    "STOA_DEV",
    "STOA_CONTAINER",
    ...
    "STOA_TIMEZONE",
    "STOA_REDACT_SECRETS",
    "STOA_BACKGROUND_NOTIFICATIONS",
    "STOA_EXEC_ASK",
    "STOA_HOME_MODE",
    "STOA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.stoa/kanban.db instead of the per-test STOA_HOME.
    "STOA_KANBAN_DB",
    "STOA_KANBAN_BOARD",
    "STOA_KANBAN_HOME",
    "STOA_KANBAN_WORKSPACES_ROOT",
    "STOA_KANBAN_LOGS_ROOT",
    "STOA_KANBAN_TASK",
    "STOA_KANBAN_WORKSPACE",
    "STOA_KANBAN_RUN_ID",
    "STOA_KANBAN_CLAIM_LOCK",
    "STOA_KANBAN_DISPATCH_IN_GATEWAY",
    "STOA_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and STOA_HOME to per-test tempdirs so code that
    reads ``~/.stoa/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral STOA_* vars that could change test semantics.
    for name in _STOA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect STOA_HOME to a per-test tempdir. Code that reads
    #    ``~/.stoa/*`` via ``get_stoa_home()`` now gets the tempdir.
    #
    ...
    (fake_stoa_home / "sessions").mkdir()
    (fake_stoa_home / "cron").mkdir()
    (fake_stoa_home / "memories").mkdir()
    (fake_stoa_home / "skills").mkdir()
    monkeypatch.setenv("STOA_HOME", str(fake_stoa_home))
    ...
    #    ~/.stoa/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import stoa_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_stoa_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal stoa config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``stoa-gateway`` process
...
# correlated with a ``tests/stoa_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> stoa-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart stoa-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting stoa/python patterns are also blocked.
    """
    ...
    def _matches_stoa_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _STOA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_stoa_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch stoa-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m stoa_cli.main".
                if (
                    "stoa" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live stoa-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting stoa/python could hit the live gateway. "
