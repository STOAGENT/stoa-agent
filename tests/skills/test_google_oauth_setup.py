...
class TestSTOAConstantsFallback:
    ...
    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_stoa_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _stoa_home.py with stoa_constants blocked."""
        monkeypatch.setitem(sys.modules, "stoa_constants", None)
        spec = importlib.util.spec_from_file_location("_stoa_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module
    ...
    def test_fallback_display_stoa_home_shortens_path(self, monkeypatch):
        ...
        monkeypatch.delenv("STOA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_stoa_home() == "~/.stoa"
    ...
    def test_delegates_to_stoa_constants_when_available(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "_stoa_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import stoa_constants
        assert module.get_stoa_home is stoa_constants.get_stoa_home
        assert module.display_stoa_home is stoa_constants.display_stoa_home
