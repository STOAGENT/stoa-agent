...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves stoa_cli.kanban_db on its own
  ...
  - worker logs are captured to STOA_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "STOA_KANBAN_TASK": task.id,
            "STOA_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="stoa_e2e_")
...
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/stoa_e2e_*/worker_*.log
    import glob
