...
- Path resolution (absolute, relative to STOA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp STOA_HOME."""
    stoa_home = tmp_path / ".hermes"
    stoa_home.mkdir()
    (stoa_home / "cron").mkdir()
    (stoa_home / "cron" / "output").mkdir()
    (stoa_home / "scripts").mkdir()
    monkeypatch.setenv("STOA_HOME", str(stoa_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "STOA_DIR", stoa_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", stoa_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", stoa_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", stoa_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("STOA_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.stoa/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("STOA_SESSION_PLATFORM") is None
        assert os.environ.get("STOA_SESSION_CHAT_ID") is None
        assert os.environ.get("STOA_SESSION_CHAT_NAME") is None
