...
from stoa_cli.profiles import _get_default_stoa_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "stoa"
        assert config.workspace_id == "stoa"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.stoa/honcho.json
        monkeypatch.setenv("STOA_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"stoa": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="stoa-agent"
        ):
            result = config.resolve_session_name("/home/user/stoa-agent/subdir")
        assert result == "stoa-agent"
...
class TestResolveConfigPath:
    def test_prefers_stoa_home_when_exists(self, tmp_path):
        stoa_home = tmp_path / "stoa"
        stoa_home.mkdir()
        local_cfg = stoa_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: STOA_HOME points at ~/.stoa/profiles/<name>, so
        # _get_default_stoa_home() must resolve back to ~/.stoa — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".stoa"
        ...
        assert _get_default_stoa_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_stoa_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".stoa".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".stoa"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_stoa(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("STOA_HONCHO_HOST", None)
            os.environ.pop("STOA_HOME", None)
            assert resolve_active_host() == "stoa"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"STOA_HONCHO_HOST": "stoa.coder"}):
            assert resolve_active_host() == "stoa.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("STOA_HONCHO_HOST", None)
            with patch("stoa_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "stoa.coder"

    def test_default_profile_returns_stoa(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("STOA_HONCHO_HOST", None)
            with patch("stoa_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "stoa"

    def test_custom_profile_returns_stoa(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("STOA_HONCHO_HOST", None)
            with patch("stoa_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "stoa"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("STOA_HONCHO_HOST", None)
            # Temporarily remove stoa_cli.profiles to simulate import failure
            saved = sys.modules.get("stoa_cli.profiles")
            sys.modules["stoa_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "stoa"
            finally:
                if saved is not None:
                    sys.modules["stoa_cli.profiles"] = saved
                else:
                    sys.modules.pop("stoa_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="stoa.coder")
        assert config.host == "stoa.coder"
        assert config.workspace_id == "stoa"  # shared workspace
        assert config.ai_peer == "stoa.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="stoa")
        assert config.host == "stoa"
        assert config.workspace_id == "stoa"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "stoa": {"aiPeer": "stoa", "peerName": "alice"},
                "stoa.coder": {
                    "aiPeer": "stoa.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="stoa.coder", config_path=config_file,
        )
        assert config.host == "stoa.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "stoa.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="stoa.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "stoa.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"stoa": {"enabled": True, "aiPeer": "stoa"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"stoa": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_stoa_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="stoa",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("stoa_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("stoa_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_stoa_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="stoa",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("stoa_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
