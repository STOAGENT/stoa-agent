...
""",
        ...
        from stoa_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        stoa_home = tmp_path / ".stoa"
        stoa_home.mkdir()
        (stoa_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("STOA_HOME", str(stoa_home))
        from stoa_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from stoa_cli.plugins import PluginContext, PluginManifest, PluginManager
