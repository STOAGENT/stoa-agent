...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. STOA passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("STOA_HOME", "/users/alice/.stoa/profiles/backend-worker")
        monkeypatch.setenv("STOA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "STOA_KANBAN_DB",
            "/users/alice/.stoa/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.stoa/kanban/boards/smoke"]'
            in cmd
        )
