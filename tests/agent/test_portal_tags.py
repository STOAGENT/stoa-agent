...
def test_stoa_client_tag_includes_current_version():
    """The client tag must reflect stoa_cli.__version__ verbatim."""
    from stoa_cli import __version__
    from agent.portal_tags import stoa_client_tag

    assert stoa_client_tag() == f"client=stoa-client-v{__version__}"


def test_stoa_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import stoa_client_tag

    tag = stoa_client_tag()
    assert tag.startswith("client=stoa-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import stoa_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=stoa-agent" in tags
    assert stoa_client_tag() in tags
    assert len(tags) == 2
