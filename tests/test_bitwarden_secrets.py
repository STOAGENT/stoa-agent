...
def stoa_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_stoa_home; clear if needed.
    import stoa_constants
    if hasattr(stoa_constants, "_STOA_HOME_CACHE"):
        stoa_constants._STOA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(stoa_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(stoa_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(stoa_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from stoa_cli.env_loader import _apply_external_secret_sources
