...
See: https://github.com/stoa-xyz/stoa-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for stoa_constants.get_subprocess_home()."""

    def test_returns_none_when_stoa_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("STOA_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("STOA_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from stoa_constants import (
            get_stoa_home,
            reset_stoa_home_override,
            set_stoa_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_stoa_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_stoa_home_override(profile)
        try:
            assert get_stoa_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_stoa_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_stoa_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_no_injection_when_stoa_home_unset(self, monkeypatch):
        monkeypatch.delenv("STOA_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["STOA_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from stoa_cli.profiles import _PROFILE_DIRS
