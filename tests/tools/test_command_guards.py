...
def _clean_state():
    ...
    saved = {}
    for k in ("STOA_INTERACTIVE", "STOA_GATEWAY_SESSION", "STOA_EXEC_ASK", "STOA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["STOA_INTERACTIVE"] = "1"
        session_key = os.getenv("STOA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No STOA_INTERACTIVE or STOA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
