...
def _restore_tool_modules():
    original_stoa_home = os.environ.get("STOA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_stoa_home is None:
            os.environ.pop("STOA_HOME", None)
        else:
            os.environ["STOA_HOME"] = original_stoa_home
        _reset_modules(("tools", "stoa_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="stoa-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": stoa_home / "modal_snapshots.json",
