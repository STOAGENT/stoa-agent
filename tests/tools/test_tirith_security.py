...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._stoa_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_stoa_bin(self):
        """After _INSTALL_FAILED, manual install in STOA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        stoa_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(stoa_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(stoa_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._stoa_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == stoa_bin
            assert _tirith_mod._resolved_path == stoa_bin
            mock_clear.assert_called_once()
...
class TestSTOAHomeIsolation:
    def test_stoa_bin_dir_respects_stoa_home(self):
        ...
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"STOA_HOME": tmpdir}):
            result = _stoa_bin_dir()
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        stoa_home = os.getenv("STOA_HOME")
        assert stoa_home is not None, "STOA_HOME should be set by conftest"
        assert "stoa_test" in stoa_home, "Should point to test temp dir"

    def test_get_stoa_home_fallback(self):
        """Without STOA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_stoa_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove STOA_HOME entirely. With HOME also absent, expanduser
