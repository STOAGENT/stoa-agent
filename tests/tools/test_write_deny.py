...
class TestWriteDenyExactPaths:
    ...
    def test_stoa_env(self):
        # ``.env`` under the active STOA_HOME (profile-aware, not just
        # ``~/.stoa``) must be write-denied. The hermetic test conftest
        # points STOA_HOME at a tempdir — resolve via get_stoa_home()
        # to match the denylist.
        from stoa_constants import get_stoa_home
        path = str(get_stoa_home() / ".env")
        assert _is_write_denied(path) is True

    def test_stoa_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.stoa/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "stoa_root"
        ...
        # Sanity check: STOA_HOME does point to the profile dir, not the root.
        from stoa_constants import get_stoa_home, get_default_stoa_root
        assert get_stoa_home() == profile_home
        assert get_default_stoa_root() == root
...
class TestWriteAllowed:
    ...
    def test_stoa_config_not_env(self):
        path = os.path.join(str(Path.home()), ".stoa", "config.yaml")
