...
class TestCronjobRequirements:
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("STOA_GATEWAY_SESSION", "1")
        monkeypatch.delenv("STOA_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("STOA_INTERACTIVE", raising=False)
        monkeypatch.delenv("STOA_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("STOA_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("STOA_INTERACTIVE", false_like_value)
        monkeypatch.delenv("STOA_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("STOA_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["STOA_INTERACTIVE", "STOA_GATEWAY_SESSION", "STOA_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("STOA_INTERACTIVE", "STOA_GATEWAY_SESSION", "STOA_EXEC_ASK"):
