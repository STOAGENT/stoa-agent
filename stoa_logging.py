"""Centralized logging setup for STOA Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.stoa/logs/`` (profile-aware via ``get_stoa_home()``).
...
from stoa_constants import get_config_path, get_stoa_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_stoa_session_injector", False):
    ...
    _session_record_factory._stoa_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``stoa logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "stoa_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the STOA logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    stoa_home
        Override for the STOA home directory.  Falls back to
        ``get_stoa_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_stoa_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._stoa_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the stoa group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from stoa_cli.config import is_managed
