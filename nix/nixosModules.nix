# nix/nixosModules.nix — NixOS module for stoa-agent
#
# Two modes:
...
# container recreation. Environment variables are written to $STOA_HOME/.env
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.stoa-agent;
    effectivePackage =
    ...
    stoa-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-stoa-agent)
    ...
    documentDerivation = pkgs.runCommand "hermes-documents" { } (
      ...
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'STOA_DOC_EOF'\n${value}\nSTOA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "stoa-agent";
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      STOA_UID="''${STOA_UID:?STOA_UID must be set}"
      STOA_GID="''${STOA_GID:?STOA_GID must be set}"

      # ── Group: ensure a group with GID=$STOA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$STOA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="hermes"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$STOA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$STOA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$STOA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$STOA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$STOA_UID" -g "$STOA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$STOA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$STOA_UID:$STOA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure STOA_HOME is owned by the target user.
      ...
      if [ -n "''${STOA_HOME:-}" ] && [ -d "$STOA_HOME" ]; then
        find "$STOA_HOME" \! -user "$STOA_UID" -exec chown "$STOA_UID:$STOA_GID" {} +
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$STOA_UID" --regid="$STOA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $STOA_HOME/.env.
  ...
  in {
    options.services.stoa-agent = with lib; {
      enable = mkEnableOption "STOA Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = stoa-agent;
        description = "The stoa-agent package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .hermes/ subdir (STOA_HOME).";
      };
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $STOA_HOME/.env at activation time.
          Hermes reads this file on every startup via load_stoa_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $STOA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $STOA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          stoa_agent.plugins entry-point group. Each package must be built
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in stoa-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        description = ''
          ...
          STOA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.stoa symlink to the service
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.stoa-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the hermes CLI to system PATH and export STOA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.stoa/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.STOA_HOME = "${cfg.stateDir}/.hermes";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.stoa-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.stoa-agent: container.enable is true and container.hostUsers
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.stoa/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.stoa/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.stoa/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.stoa/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.stoa/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."stoa-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.stoa/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.stoa/$_subdir"
            chmod 2770 "${cfg.stateDir}/.stoa/$_subdir"
            find "${cfg.stateDir}/.stoa/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.stoa/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.stoa/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.stoa/config.yaml
            chmod 0640 ${cfg.stateDir}/.stoa/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.stoa/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.stoa/.managed
          chmod 0644 ${cfg.stateDir}/.stoa/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.stoa/.container-mode <<'STOA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    stoa_bin=${containerDataDir}/current-package/bin/hermes
    STOA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.stoa/.container-mode
            chmod 0644 ${cfg.stateDir}/.stoa/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.stoa/.container-mode
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.hermes" ]; then
                  rm -f "${symlinkPath}"
                  echo "stoa-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.stoa -> stateDir/.hermes for each hostUser so the
            ...
            (lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  ...
                  echo "stoa-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.stoa/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.stoa/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.stoa/auth.json
              fi
            ''}
          ''}
          ...
          # Hermes reads $STOA_HOME/.env at startup via load_stoa_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.stoa/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'STOA_NIX_ENV_EOF'
    ${envFileContent}
    STOA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.stoa/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.stoa/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.stoa/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.stoa-agent = {
          description = "STOA Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $STOA_HOME/.env by the activation script. load_stoa_dotenv()
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.stoa-agent = {
          description = "STOA Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              STOA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              STOA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env STOA_UID="$STOA_UID" \
                --env STOA_GID="$STOA_GID" \
                --env STOA_HOME=${containerDataDir}/.hermes \
                --env STOA_MANAGED=true \
