...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      stoa-agent = self'.packages.default;
      hermesVenv = stoa-agent.hermesVenv;
    ...
    in {
      ...
      checks = {
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${stoa-agent}/bin/hermes || (echo "FAIL: hermes binary missing"; exit 1)
          test -x ${stoa-agent}/bin/stoa-agent || (echo "FAIL: stoa-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${stoa-agent}/bin/hermes version 2>&1 | grep -qi "hermes" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hermes-entry-points-sync" { } ''
          ...
          for bin in hermes stoa-agent hermes-acp; do
            test -x ${stoa-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "stoa-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking hermes --help ==="
          ${stoa-agent}/bin/hermes --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${stoa-agent}/bin/hermes --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          test -d ${stoa-agent}/share/stoa-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${stoa-agent}/share/stoa-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "STOA_BUNDLED_SKILLS" ${stoa-agent}/bin/hermes || \
            (echo "FAIL: STOA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: STOA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          test -d ${stoa-agent}/share/stoa-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${stoa-agent}/share/stoa-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "STOA_BUNDLED_PLUGINS" ${stoa-agent}/bin/hermes || \
            (echo "FAIL: STOA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: STOA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          test -d ${stoa-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${stoa-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "STOA_TUI_DIR" ${stoa-agent}/bin/hermes || \
            (echo "FAIL: STOA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: STOA_TUI_DIR set in wrapper"
        ...
        # Verify STOA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          set -e
          echo "=== Checking STOA_NODE in wrapper ==="
          grep -q "STOA_NODE" ${stoa-agent}/bin/hermes || \
            (echo "FAIL: STOA_NODE not set in wrapper"; exit 1)
          echo "PASS: STOA_NODE present in wrapper"

          STOA_NODE=$(sed -n "s/^export STOA_NODE='\(.*\)'/\1/p" ${stoa-agent}/bin/hermes)
          test -x "$STOA_NODE" || (echo "FAIL: STOA_NODE=$STOA_NODE not executable"; exit 1)
          echo "PASS: STOA_NODE executable at $STOA_NODE"

          NODE_MAJOR=$("$STOA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All STOA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify STOA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(STOA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking STOA_MANAGED guards ==="
          check_blocked "config set" ${stoa-agent}/bin/hermes config set model foo
          check_blocked "config edit" ${stoa-agent}/bin/hermes config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = stoa-agent.override {
        ...
        in pkgs.runCommand "hermes-extra-python-packages" { } ''
          ...
          if grep -q "PYTHONPATH" ${stoa-agent}/bin/hermes; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = stoa-agent.override {
        ...
        in pkgs.runCommand "hermes-config-roundtrip" {
          ...
          merge_and_load() {
            local stoa_home="$1"
            export STOA_HOME="$stoa_home"
            ${configMergeScript} ${nixSettings} "$stoa_home/config.yaml"
            ${hermesVenv}/bin/python3 -c '
import json, sys
