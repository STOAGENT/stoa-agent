"""Persistent slash-command worker — one STOACLI per TUI session.
...
def _run(cli: STOACLI, command: str) -> str:
...
def main():
    ...
    os.environ["STOA_SESSION_KEY"] = args.session_key
    os.environ["STOA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = STOACLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
