#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the STOA docs site.
...
Both publish at:
  https://stoa-agent.nousresearch.com/docs/llms.txt
  https://stoa-agent.nousresearch.com/docs/llms-full.txt
...
SITE_BASE = "https://stoa-agent.nousresearch.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use STOA as a Python Library", None),
        ("guides/use-mcp-with-stoa", "Use MCP with STOA", None),
        ("guides/use-voice-mode-with-stoa", "Use Voice Mode with STOA", None),
        ("guides/use-soul-with-stoa", "Use SOUL.md with STOA", None),
        ("guides/build-a-stoa-plugin", "Build a STOA Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with STOA"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# STOA Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by STOA. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/stoa-xyz/"
        "stoa-agent/main/scripts/install.sh | bash`  "
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# STOA Agent — Full Documentation\n",
        (
            "This file is the entire STOA Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using STOA, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://stoa-agent.nousresearch.com/docs\n",
        "Short index: https://stoa-agent.nousresearch.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
