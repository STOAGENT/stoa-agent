...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/STOAGENT/stoa-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    stoa_meta = (fm.get("metadata") or {}).get("stoa") or {}
    tags = stoa_meta.get("tags") or []
    related = stoa_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `stoa skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with STOA Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "STOA ships with a large built-in skill library copied into `~/.stoa/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "STOA also syncs bundled skills on `stoa update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.stoa/skills/` tree, it is still shipped with STOA; restore it with `stoa skills reset <name> --restore`.",
        "",
