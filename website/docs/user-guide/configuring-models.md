...
STOA uses two kinds of model slots:
...
[Nous Portal](/docs/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `stoa setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `stoa portal status`.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models STOA recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and STOA writes it to `~/.stoa/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning STOA uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `stoa skills search` uses this. Usually fine at `auto`. |
...
When you save via the dashboard, STOA writes to `~/.stoa/config.yaml`:
...
`provider: auto` with `model: ''` tells STOA to use the main model for that task.

## When does it take effect?

- **CLI** (`stoa chat`): next `stoa chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`stoa gateway restart`) if you want to force all sessions to pick up the change.
...
STOA lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `stoa setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but STOA switched providers on me
...
```bash
stoa config set model.aliases.fav anthropic/claude-opus-4.6
stoa config set model.aliases.grok x-ai/grok-4
```
...
### `stoa model` subcommand

```bash
stoa model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`stoa model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.stoa/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `stoa config get model` and `stoa status`.

### Direct config edit

Edit `~/.stoa/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-STOA-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-STOA-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-STOA-Session-Token: $TOKEN" \
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__STOA_SESSION_TOKEN__`) if you're scripting against a running dashboard.
