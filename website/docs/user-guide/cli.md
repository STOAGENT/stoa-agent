...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `STOA_YOLO_MODE` is on (either `hermes --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
CLI sessions are stored in Hermes's SQLite state database under `~/.stoa/state.db`. The database keeps:
