...
The Tool Gateway is included with every paid [Nous Portal](https://portal.nousresearch.com) subscription. It routes STOA' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Nous already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let STOA default to FLUX 2 Klein. |
...
```bash
stoa setup --portal     # Nous OAuth, set Nous as provider, and turn on the Tool Gateway in one go
```

Already have STOA configured? Just switch your provider:

```bash
stoa model              # Pick Nous Portal — STOA will offer to turn on the Tool Gateway
```

When you select Nous Portal, STOA offers to turn on the Tool Gateway. Accept, and you're done — every supported tool is live on the next run.

Check what's active at any time:

```bash
stoa portal status      # Portal auth + Tool Gateway routing summary
stoa portal tools       # Gateway catalog with current routing per tool
stoa status             # Full system status (Tool Gateway is one section)
```

`stoa portal status` shows a section like:
...
```bash
stoa tools          # Interactive picker for each tool category
```
...
The set evolves — `stoa tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `stoa model` and `stoa tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.
...
```yaml
web:
  use_gateway: false   # STOA now uses FIRECRAWL_API_KEY from .env
```

`stoa tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.

### Self-hosted gateway (advanced)

Running your own Nous-compatible gateway? Override endpoints in `~/.stoa/.env`:
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `stoa tools`. STOA shows a clear error pointing at the portal.
...
Modal is available as an **optional add-on** through the Nous subscription, not part of the default Tool Gateway bundle. Configure it via `stoa setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. When `use_gateway: true`, STOA skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
