...
When STOA Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.stoa/config.yaml` under `agent.custom_instructions`.
...
Install MCP servers via `~/.stoa/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. STOA Agent's deliverable
