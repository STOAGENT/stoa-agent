...
chat completions — use your STOA-managed provider subscription as their
...
| Use case | "Use STOA as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. STOA stores
the refresh token in `~/.stoa/auth.json` — the same place all STOA
...
```
Starting STOA proxy for Nous Portal
...
Model:      STOA-4-70B    # or STOA-4.3-36B, STOA-4-405B
```
...
```
STOA proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `stoa login nous`. If you see
...
INFERENCE_TEXT_MODEL=STOA-4-70B
```
...
```bash
stoa proxy start --host 0.0.0.0 --port 8645
```
...
`stoa_cli/proxy/adapters/<provider>.py` and registering it in
