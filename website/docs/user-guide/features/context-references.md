...
Type `@` followed by a reference to inject content directly into your message. STOA expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- STOA env: `$STOA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$STOA_HOME/skills/.hub/`
