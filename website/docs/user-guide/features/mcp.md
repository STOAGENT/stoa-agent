...
description: "Connect STOA Agent to external tool servers via MCP — and control exactly which MCP tools STOA loads"
---

# MCP (Model Context Protocol)

MCP lets STOA Agent connect to external tool servers so the agent can use tools that live outside STOA itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted STOA to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native STOA tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want STOA to see
...
2. Add an MCP server to `~/.stoa/config.yaml`:
...
4. Ask STOA to use the MCP-backed capability.
...
STOA will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints STOA connects to directly.
...
- you do not want STOA spawning a local subprocess for that integration

## Basic configuration reference

STOA reads MCP config from `~/.stoa/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, STOA skips the server entirely |
...
For well-known MCP servers, `stoa mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
stoa mcp add codex --preset codex
```
...
You can pick any local name (`stoa mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How STOA registers MCP tools

STOA prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — STOA sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, STOA also registers utility tools around MCP resources and prompts:
...
- STOA only registers resource utilities if the MCP session actually supports resource operations
- STOA only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to STOA, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, STOA skips the server completely and does not even attempt a connection.
...
You can also separately disable STOA-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, STOA does not create an empty runtime MCP toolset for that server.
...
STOA discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify STOA when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When STOA receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, STOA does not blindly pass your full shell environment.
...
Then verify your config and restart STOA.
...
Because STOA now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, STOA may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from STOA via the `sampling/createMessage` protocol. This allows an MCP server to ask STOA to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running STOA as an MCP server

In addition to connecting **to** MCP servers, STOA can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use STOA's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through STOA
- You want a single MCP server that bridges to all of STOA's connected messaging platforms at once
- You already have a running STOA gateway with connected platforms
...
Add STOA to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed STOA in a specific location:

```json
{
  "mcpServers": {
    "stoa": {
      "command": "/home/user/.stoa/stoa-agent/venv/bin/stoa",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a STOA-specific channel browser:
...
The MCP server includes a live event bridge that polls STOA's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
stoa mcp serve              # Normal mode
stoa mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from STOA's session store (`~/.stoa/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the STOA agent itself.
...
- The embedded `stoa mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of STOA, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
