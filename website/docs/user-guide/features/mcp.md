...
description: "Connect STOA Agent to external tool servers via MCP — and control exactly which MCP tools Hermes loads"
---

# MCP (Model Context Protocol)

MCP lets STOA Agent connect to external tool servers so the agent can use tools that live outside Hermes itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.
...
Hermes reads MCP config from `~/.stoa/config.yaml` under `mcp_servers`.
...
```json
{
  "mcpServers": {
    "hermes": {
      "command": "/home/user/.stoa/stoa-agent/venv/bin/hermes",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server reads conversation data directly from Hermes's session store (`~/.stoa/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Hermes agent itself.
