...
Hermes ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.stoa/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.
...
| `stoa-achievements` | dashboard tab | Steam-style collectible badges generated from your real Hermes session history |
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `STOA_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `STOA_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
Or check the box in the interactive `hermes plugins` UI. Then put the credentials in `~/.stoa/.env`:
...
**Disabling:** `hermes plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.stoa/cache/google_meet/` until you remove them.
