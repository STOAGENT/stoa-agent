...
Drop both files into `~/.stoa/plugins/hello-world/`, restart Hermes, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.stoa/plugins/` are disabled by default. Enable them only for trusted repositories by setting `STOA_ENABLE_PROJECT_PLUGINS=true` before starting Hermes.
...
**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `hermes plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.stoa/config.yaml`. This stops third-party code from running without your explicit consent.
