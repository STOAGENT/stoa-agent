...
STOA runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. STOA
...
1. STOA captures a baseline of current diagnostics for the file.
...
…). STOA auto-detects the binary on PATH or in
`<STOA_HOME>/lsp/bin/`.
...
`node_modules` tree — STOA installs both packages together when you
run `stoa lsp install typescript` or auto-install fires on first
use.

## CLI

```
stoa lsp status          # service state + per-server install status
stoa lsp list            # registry, optionally --installed-only
stoa lsp install <id>    # eagerly install one server
stoa lsp install-all     # try every server with a known recipe
stoa lsp restart         # tear down running clients
stoa lsp which <id>      # print resolved binary path
```

`stoa lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <STOA_HOME>/lsp/bin
...
When `install_strategy: auto`, STOA installs binaries into
`<STOA_HOME>/lsp/bin/`. NPM packages land in
`<STOA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully STOA-owned and is
...
Servers are kept alive for the life of the STOA process. There's
...
**`stoa lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<STOA_HOME>/lsp/bin/`. Run
`stoa lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `stoa lsp status`**
...
When `stoa lsp status` shows a `Backend warnings` section, install
...
Check `~/.stoa/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `stoa lsp restart` to clear the set;
