...
The tool surfaces this when both auth paths fail. Either set `XAI_API_KEY` in `~/.stoa/.env` or run `hermes auth add xai-oauth` and complete the browser login. Then restart your session so the agent re-reads the tool registry.
