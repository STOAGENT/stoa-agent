...
By default a cron job inherits whichever Hermes profile owned the gateway / CLI that created it. Pass `--profile <name>` (CLI) or `profile=` (cronjob tool) to re-target the job at a different profile — the scheduler resolves that profile's `STOA_HOME`, temporarily switches into it for the duration of the run, loads its `.env` + `config.yaml`, and executes the job there:
...
Hermes will write the check script to `~/.stoa/scripts/` via `write_file`, then call:
...
- When Job 2 fires, Hermes reads Job 1's most recent output from `~/.stoa/cron/output/{job1_id}/*.md`
...
:::tip
Hermes's own `~/.stoa/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::
