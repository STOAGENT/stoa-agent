...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of STOA's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple STOA instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
stoa memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$STOA_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `stoa` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — STOA syncs them back at session init.
...
The `stoa honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `stoa memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
stoa honcho status          # Connection status, config, and key settings
stoa honcho setup           # Redirects to `stoa memory setup`
stoa honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
stoa honcho peer            # Show or update peer names + dialectic reasoning level
stoa honcho mode            # Show or set recall mode (hybrid/context/tools)
stoa honcho tokens          # Show or set token budget for context and dialectic
stoa honcho identity        # Seed or show the AI peer's Honcho identity
stoa honcho sync            # Sync Honcho config to all existing profiles
stoa honcho peers           # Show peer identities across all profiles
stoa honcho sessions        # List known Honcho session mappings
stoa honcho map             # Map current directory to a Honcho session name
stoa honcho enable          # Enable Honcho for the active profile
stoa honcho disable         # Disable Honcho for the active profile
stoa honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `stoa honcho`

If you previously used the standalone `stoa honcho setup`:
...
No re-login or re-setup needed. Run `stoa memory setup` and select "honcho" — the wizard detects your existing config.
