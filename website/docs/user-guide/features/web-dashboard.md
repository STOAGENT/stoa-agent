...
| `--tui` | off | Expose the in-browser Chat tab (embedded `hermes --tui` via PTY/WebSocket). Alternatively set `STOA_DASHBOARD_TUI=1`. |
...
The Chat tab is intentionally off for a plain `hermes dashboard` launch. Start the dashboard with `hermes dashboard --tui` or set `STOA_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
