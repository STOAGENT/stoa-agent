...
When the model wants one of these, codex spawns the `stoa_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Hermes' default runtime), and the result is returned to codex like any other MCP response.
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Hermes' own `hermes auth login codex` writes to `~/.stoa/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
(Hermes will preserve your override on re-migration as long as it lives outside the `# managed by stoa-agent` markers.)
...
```toml
# managed by stoa-agent — `hermes codex-runtime migrate` regenerates this section
...
CODEX_HOME=~/.stoa/profiles/work/codex hermes chat
```
...
When the model calls `web_search` (or another exposed Hermes tool), codex spawns the `stoa_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
If you find a bug, [open an issue](https://github.com/stoa-xyz/stoa-agent/issues) with the output of `hermes logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
...
For implementation details, see [PR #24182](https://github.com/stoa-xyz/stoa-agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
