...
description: "Customize STOA Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

STOA Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `STOA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who STOA is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

STOA now seeds a default `SOUL.md` automatically in:

```text
~/.stoa/SOUL.md
```

More precisely, it uses the current instance's `STOA_HOME`, so if you run STOA with a custom home directory, it will use:
...
- STOA creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- STOA loads `SOUL.md` only from `STOA_HOME`
- STOA does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, STOA falls back to a built-in default identity
...
If STOA loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `STOA_HOME`, the personality belongs to the STOA instance itself.

That also makes it easier to teach users:
- "Edit `~/.stoa/SOUL.md` to change STOA' default personality."
...
- how STOA should handle uncertainty, disagreement, or ambiguity
...
## What STOA injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, STOA falls back to a built-in default identity ("You are STOA Agent, an intelligent AI assistant created by STOA..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
STOA ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain STOA, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives STOA its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.stoa/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.stoa/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-stoa)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how STOA speaks
- `display.skin` and `/skin` affect how STOA looks in the terminal
