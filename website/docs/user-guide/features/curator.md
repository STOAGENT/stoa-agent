...
Tracks [issue #7816](https://github.com/stoa-xyz/stoa-agent/issues/7816).
...
hermes curator backup         # take a manual snapshot of ~/.stoa/skills/
...
Before every real curator pass, Hermes takes a tar.gz snapshot of `~/.stoa/skills/` at `~/.stoa/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
...
- `~/.stoa/skills/.hub/lock.json` (skills installed via `hermes skills install`).
...
- [Issue #7816](https://github.com/stoa-xyz/stoa-agent/issues/7816) — original proposal and design discussion
