...
All skills live in **`~/.stoa/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point STOA at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling STOA to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.stoa/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
stoa chat --toolsets skills -q "What skills do you have?"
stoa chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.stoa/cache/chart-q4-2025.png
...
When a missing value is encountered, STOA asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `stoa setup` or `~/.stoa/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `stoa config migrate` prompts for unconfigured settings, and `stoa config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.stoa/skills/                  # Single source of truth
...
If you maintain skills outside of STOA — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell STOA to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.stoa/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.stoa/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the STOA process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, STOA ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.stoa/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.stoa/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `stoa bundles list`.
...
stoa bundles create research

# Overwrite an existing bundle
stoa bundles create backend-dev --skill ... --force

# Delete a bundle
stoa bundles delete backend-dev

# Re-scan ~/.stoa/skill-bundles/ and report changes
stoa bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.stoa/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.stoa/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
stoa skills browse                              # Browse all hub skills (official first)
stoa skills browse --source official            # Browse only official optional skills
stoa skills search kubernetes                   # Search all sources
stoa skills search react --source skills-sh     # Search the skills.sh directory
stoa skills search https://mintlify.com/docs --source well-known
stoa skills inspect openai/skills/k8s           # Preview before installing
stoa skills install openai/skills/k8s           # Install with security scan
stoa skills install official/security/1password
stoa skills install skills-sh/vercel-labs/json-render/json-render-react --force
stoa skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
stoa skills list --source hub                   # List hub-installed skills
stoa skills check                               # Check installed hub skills for upstream updates
stoa skills update                              # Reinstall hub skills with upstream changes when needed
stoa skills audit                               # Re-scan all hub skills for security
stoa skills uninstall k8s                       # Remove a hub skill
stoa skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
stoa skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
stoa skills publish skills/my-skill --to github --repo owner/repo
stoa skills snapshot export setup.json          # Export skill config
stoa skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with STOA. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `stoa skills search <query> --source skills-sh`. STOA resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
STOA currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the STOA repository itself and install with builtin trust.
...
This is Vercel's public skills directory. STOA can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
stoa skills search https://mintlify.com/docs --source well-known
stoa skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
stoa skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

STOA can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
stoa skills install openai/skills/k8s
stoa skills tap add myorg/skills-repo
```
...
- STOA source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

STOA supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

STOA source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

STOA can search and convert agent entries from LobeHub's public catalog into installable STOA skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- STOA source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

STOA integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with STOA' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- STOA source id: `browse-sh`
- Trust level: `community`

```bash
stoa skills search airbnb --source browse-sh
stoa skills inspect browse-sh/airbnb.com/search-listings-ddgioa
stoa skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). STOA fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
stoa skills install https://sharethis.chat/SKILL.md
stoa skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
stoa skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `stoa skills update` re-fetches from the same URL automatically when you want to refresh.
...
`stoa skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with STOA | Always trusted |
...
```bash
stoa skills check          # Report which installed hub skills changed upstream
stoa skills update         # Reinstall only the skills with updates available
stoa skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other STOA users add with `stoa skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.stoa.tags`, `version`, `author`, `platforms`, `metadata.stoa.config`).
...
STOA discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any STOA user can subscribe and install:

```bash
stoa skills tap add my-org/stoa-skills
stoa skills search deploy
stoa skills install my-org/stoa-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.stoa/.hub/taps.json`:
...
The `stoa skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `stoa skills tap list` shows the effective path per tap.
...
```bash
stoa skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.stoa/.hub/taps.json` (created on demand).

## Bundled skill updates (`stoa skills reset`)

STOA ships with a set of bundled skills in `skills/` inside the repo. On install and on every `stoa update`, a sync pass copies those into `~/.stoa/skills/` and records a manifest at `~/.stoa/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, STOA recomputes the hash of your local copy and compares it to the origin hash:
...
stoa skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `STOA_HOME`, so `stoa -p coder skills reset <name>` only affects that profile.
:::
