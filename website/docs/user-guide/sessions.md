...
STOA Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.stoa/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
STOA stores session history so it can resume conversations, but it does not
...
STOA explicitly injects for that turn.
...
For example, if a user sends an image and asks STOA to make a meme from it,
STOA may inspect that image once with vision and run an image-processing
...
`stoa sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`stoa` or `stoa chat`) |
...
stoa -c "my project"   # → resumes "my project #3"
```
...
stoa --resume 20250305_091523_a1b2c3d4
stoa -r 20250305_091523_a1b2c3d4

# Resume by title
stoa --resume "refactoring auth"

# Or with the chat subcommand
stoa chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `stoa sessions list`.

### Conversation Recap on Resume

When you resume a session, STOA displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a STOA session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.stoa/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `stoa -r "<title>"` from the shell) and pick up where the platform left off.
...
STOA automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `stoa sessions list` or `stoa sessions browse`.
...
```bash
stoa sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), STOA creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`stoa -c "my project"`), it automatically picks the most recent session in the lineage.
...
STOA provides a full set of session management commands via `stoa sessions`:
...
# Filter by platform
stoa sessions list --source telegram

# Show more sessions
stoa sessions list --limit 50
```
...
stoa sessions export backup.jsonl

# Export sessions from a specific platform
stoa sessions export telegram-history.jsonl --source telegram

# Export a single session
stoa sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
stoa sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
stoa sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
stoa sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
stoa sessions prune --older-than 30

# Only prune sessions from a specific platform
stoa sessions prune --source telegram --older-than 60

# Skip confirmation
stoa sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`stoa insights`](/docs/reference/cli-commands#stoa-insights).
...
When STOA cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, STOA uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to STOA in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.stoa/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.stoa/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.stoa/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.stoa/sessions/`. They are no longer written or
read by STOA. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every STOA process in the same `STOA_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.stoa/config.yaml`:
...
stoa sessions delete <session_id>
