...
| | |
|---|---|
| Source | Optional — install with `stoa skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${STOA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${STOA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${STOA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client stoa
```
...
```bash
PUBLISH="${STOA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client stoa
```
...
```bash
DRIVE="${STOA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `stoa`)    |
