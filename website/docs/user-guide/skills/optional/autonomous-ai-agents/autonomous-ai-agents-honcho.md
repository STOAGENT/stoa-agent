...
description: "Configure and use Honcho memory with STOA -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with STOA -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `stoa skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for STOA

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every STOA profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/stoa#running-honcho-locally-with-stoa

### Verify

```bash
stoa honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this STOA profile's AI peer
...
Honcho models conversations as interactions between **peers**. STOA creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this STOA instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each STOA run |
| `global` | Single session across all directories |

Manual override: `stoa honcho map my-project-name`
...
Each STOA profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block stoa.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `stoa.coder` host block in `honcho.json`
...
```bash
stoa honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for STOA when Honcho memory is active.
...
Config file: `$STOA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `stoa honcho setup`. Ensure `memory.provider: honcho` is in `~/.stoa/config.yaml`.

### Memory not persisting across sessions
Check `stoa honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `stoa profile create <name> --clone`. For existing profiles: `stoa honcho sync`.
...
| Command | Description |
|---------|-------------|
| `stoa honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `stoa honcho status` | Show resolved config, connection test, peer info for active profile |
| `stoa honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `stoa honcho disable` | Disable Honcho for the active profile |
| `stoa honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `stoa honcho peers` | Show peer identities across all profiles |
| `stoa honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `stoa honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `stoa honcho sessions` | List known directory-to-session-name mappings |
| `stoa honcho map <name>` | Map current working directory to a Honcho session name |
| `stoa honcho identity` | Seed AI peer identity or show both peer representations |
| `stoa honcho sync` | Create host blocks for all STOA profiles that don't have one yet |
| `stoa honcho migrate` | Step-by-step migration guide from OpenClaw native memory to STOA + Honcho |
| `stoa memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `stoa memory status` | Show active memory provider and config |
| `stoa memory off` | Disable external memory provider |
