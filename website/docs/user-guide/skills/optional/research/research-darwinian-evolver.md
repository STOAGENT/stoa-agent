...
| | |
|---|---|
| Source | Optional — install with `stoa skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), STOA Agent |
| License | MIT |
...
import upstream classes into STOA itself.
...
```bash
mkdir -p ~/.stoa/cache/darwinian-evolver && cd ~/.stoa/cache/darwinian-evolver
...
```bash
cd ~/.stoa/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.stoa/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.stoa/skills/research/darwinian-evolver
DE_DIR=~/.stoa/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside STOA core.
   Custom driver scripts under `~/.stoa/skills/...` are user-side and fine.
