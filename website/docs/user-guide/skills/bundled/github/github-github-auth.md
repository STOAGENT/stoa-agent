...
- Give it a name like "stoa-agent"
- Select scopes:
...
- Give it a title like "stoa-agent-&lt;machine-name>"
...
elif [ -f ~/.stoa/.env ] && grep -q "^GITHUB_TOKEN=" ~/.stoa/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.stoa/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
