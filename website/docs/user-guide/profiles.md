...
:::note
You cannot delete the default profile (`~/.stoa`). To remove everything, use `hermes uninstall`.
:::
...
Profiles use the `STOA_HOME` environment variable. When you run `coder chat`, the wrapper script sets `STOA_HOME=~/.stoa/profiles/coder` before launching hermes. Since 119+ files in the codebase resolve paths via `get_stoa_home()`, Hermes state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
