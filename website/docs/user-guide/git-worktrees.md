...
title: "Git Worktrees"
description: "Run multiple STOA agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

STOA Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with STOA so each session has a clean, isolated working directory.

## Why Use Worktrees with STOA?

STOA treats the **current working directory** as the project root:

- CLI: the directory where you run `stoa` or `stoa chat`
...
git worktree add ../repo-feature feature/stoa-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/stoa-experiment` checked out in that directory

Now you can `cd` into the new worktree and run STOA there:

```bash
cd ../repo-feature

# Start STOA in the worktree
stoa
```
...
git worktree add ../repo-experiment-a feature/stoa-a
git worktree add ../repo-experiment-b feature/stoa-b
```
...
- Works on its own branch (`feature/stoa-a` vs `feature/stoa-b`).
...
- STOA checkpoint data under `~/.stoa/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per STOA experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/stoa-checkpoints-docs`, `feature/stoa-refactor-tests`.
- **Commit frequently**
...
- **Avoid running STOA from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `stoa -w` (Automatic Worktree Mode)

STOA has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `stoa/stoa-<hash>`).
...
```bash
stoa -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `stoa -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each STOA session its own clean checkout.
