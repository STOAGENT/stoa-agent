...
sidebar_position: 7
title: "Email"
description: "Set up STOA Agent as an email assistant via IMAP/SMTP"
---

# Email Setup

STOA can receive and reply to emails using standard IMAP and SMTP protocols. Send an email to the agent's address and it replies in-thread — no special client or bot API needed. Works with Gmail, Outlook, Yahoo, Fastmail, or any provider that supports IMAP/SMTP.
...
- **A dedicated email account** for your STOA agent (don't use your personal email)
...
Email access follows the same pattern as all other STOA platforms:
...
| **Duplicate replies** | Ensure only one gateway instance is running. Check `stoa gateway status`. |
...
- The password is stored in `~/.stoa/.env` — protect this file (`chmod 600`)
