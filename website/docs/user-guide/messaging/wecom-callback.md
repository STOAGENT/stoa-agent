...
Connect STOA to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
STOA supports two WeCom integration modes:
...
3. STOA decrypts the message, queues it for the agent
...
(Use `stoa gateway start` only after `stoa gateway install` has registered the systemd/launchd service.)
