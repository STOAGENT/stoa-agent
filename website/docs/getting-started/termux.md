...
description: "Run STOA Agent directly on an Android phone with Termux"
---

# STOA on Android with Termux

This is the tested path for running STOA Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop STOA from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

STOA now ships a Termux-aware installer path:
...
- links `stoa` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `stoa` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/stoa" "$PREFIX/bin/stoa"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `stoa` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.stoa/.env`.
...
### `stoa doctor` says ripgrep or Node is missing
