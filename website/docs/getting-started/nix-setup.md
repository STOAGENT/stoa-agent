...
After `nix profile install`, `hermes`, `stoa-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `hermes setup` walks you through provider selection, `hermes gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.stoa/`.
...
```nix
services.stoa-agent.environmentFiles = [ "/var/lib/hermes/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `STOA_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `~/.stoa/` directory.
:::
...
- For developers working on the hermes codebase, set `STOA_DEV=1` to bypass container routing and run the local checkout directly
...
```nix
services.stoa-agent.configFile = /etc/hermes/config.yaml;
```
...
| Override the hermes package | `package` | `inputs.stoa-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$STOA_HOME/.env` at activation time (`nixos-rebuild switch`). Hermes reads this file on every startup, so changes take effect with a `systemctl restart stoa-agent` — no container recreation needed.
...
The agent identity file is separate: Hermes loads its primary `SOUL.md` from `$STOA_HOME/SOUL.md`, which in the NixOS module is `${services.stoa-agent.stateDir}/.stoa/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
scp ~/.stoa/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/hermes/.stoa/mcp-tokens/
...
2. **`.managed` marker file** in `STOA_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it stoa-agent hermes config set ...` is also blocked)
...
Plugins are symlinked into `$STOA_HOME/plugins/` at activation time. Hermes discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."stoa_agent.plugins"]` (e.g., [rtk-hermes](https://github.com/ogallotti/rtk-hermes)):
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `stoa-agent` binaries exist and `hermes version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `hermes --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `STOA_MANAGED=true hermes config set ...` prints the NixOS error |
...
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`STOA_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory (`MESSAGING_CWD`) |
| `addToSystemPackages` | `bool` | `false` | Add `hermes` CLI to system PATH and set `STOA_HOME` system-wide |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.stoa` symlink to the service stateDir and are auto-added to the `hermes` group |
...
```bash
# Native mode
sudo -u hermes cat /var/lib/hermes/.stoa/.env
...
| `hermes version` shows old version | Container not restarted | `systemctl restart stoa-agent` |
