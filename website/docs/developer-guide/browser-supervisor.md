...
[PR #12550](https://github.com/STOAGENT/stoa-agent/pull/12550) proposed a
...
(`stoa-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per STOA `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `stoa-acp`, `stoa-api-server`, core toolsets (gated on CDP reachability)
...
- `stoa_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
