...
STOA Agent uses a SQLite database (`~/.stoa/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `stoa_state.py`


## Architecture Overview

```
~/.stoa/state.db (SQLite, WAL mode)
...
Multiple stoa processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.stoa/state.db
...
Default path: `~/.stoa/state.db`

This is derived from `stoa_constants.get_stoa_home()` which resolves to
`~/.stoa/` by default, or the value of `STOA_HOME` environment variable.
