...
description: "How STOA stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `stoa_cli/cron.py` | CLI `stoa cron` subcommands |
...
Jobs are stored in `~/.stoa/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `stoa cron` commands are run or during active CLI sessions.
...
```python
# ~/.stoa/scripts/check_competitors.py
...
2. **Environment variable** — `STOA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.stoa/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `stoa cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `stoa cron` CLI provides direct job management:

```bash
stoa cron list                    # Show all jobs
stoa cron create                  # Interactive job creation (alias: add)
stoa cron edit <job_id>           # Edit job configuration
stoa cron pause <job_id>          # Pause a running job
stoa cron resume <job_id>         # Resume a paused job
stoa cron run <job_id>            # Trigger immediate execution
stoa cron remove <job_id>         # Delete a job
```
