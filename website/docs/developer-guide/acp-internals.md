...
The ACP adapter wraps STOA' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
stoa acp / stoa-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.stoa/.env
  -> configure stderr logging
  -> construct STOAACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'stoa-agent[acp]==<version>' stoa-acp`, pointed at the `stoa-agent` PyPI release.
...
- `allow_once` -> STOA `once`
- `allow_always` -> STOA `always`
- reject options -> STOA `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps STOA tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["stoa-acp"])
...
Instead it reuses STOA' runtime resolver:

- `acp_adapter/auth.py`
- `stoa_cli/runtime_provider.py`

So ACP advertises and uses the currently configured STOA provider/credentials. It also always advertises a terminal setup auth method (`stoa-setup`, args `--setup`) so first-run registry clients can open STOA' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.stoa/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `stoa-acp` toolset definition
- `stoa_cli/main.py` — `stoa acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `stoa-acp` script
