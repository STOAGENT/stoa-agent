...
The Zed ACP Registry path launches the same adapter through `uvx --from 'stoa-agent[acp]==<version>' hermes-acp`, pointed at the `stoa-agent` PyPI release.
...
- `stoa_cli/main.py` — `hermes acp` CLI subcommand
