...
Most users set up a platform by dropping env vars into `~/.stoa/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `hermes gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
1. **`toolsets.py`** — Add `"hermes-newplat"` toolset definition with `_STOA_CORE_TOOLS`
2. **`toolsets.py`** — Add `"hermes-newplat"` to the `"stoa-gateway"` includes list
