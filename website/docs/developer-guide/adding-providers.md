...
description: "How to add a new inference provider to STOA Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

STOA can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `stoa model` menu entries
...
1. `stoa_cli/auth.py` decides how credentials are found.
2. `stoa_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `stoa_cli/models.py` and `stoa_cli/main.py` make the provider show up in the CLI. (`stoa_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `stoa_cli/auth.py`
2. `stoa_cli/models.py`
3. `stoa_cli/runtime_provider.py`
4. `stoa_cli/main.py`
...
:::tip
`stoa_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `stoa setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$STOA_HOME/plugins/model-providers/` both get picked up.
...
7. `stoa model` menu includes the provider
8. `stoa setup` wizard delegates to `main.py` automatically
...
User plugins at `$STOA_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `stoa model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `stoa_cli/auth.py`
- `_PROVIDER_LABELS` in `stoa_cli/models.py`
- `_PROVIDER_ALIASES` in both `stoa_cli/auth.py` and `stoa_cli/models.py`
- CLI `--provider` choices in `stoa_cli/main.py`
...
## Step 2: Add auth metadata in `stoa_cli/auth.py`
...
- What env vars should STOA check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `stoa_cli/models.py`
...
## Step 4: Resolve runtime data in `stoa_cli/runtime_provider.py`
...
Be careful with API-key precedence. STOA already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `stoa_cli/main.py`

A provider is not discoverable until it shows up in the interactive `stoa model` flow.

Update these in `stoa_cli/main.py`:
...
:::tip
`stoa_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `stoa model` and `stoa setup` automatically.
:::
...
When you add a native provider, double-check that STOA is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m stoa_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m stoa_cli.main model
python -m stoa_cli.main setup
```
...
- [ ] `ProviderConfig` added in `stoa_cli/auth.py`
- [ ] aliases added in `stoa_cli/auth.py` and `stoa_cli/models.py`
- [ ] model catalog added in `stoa_cli/models.py`
- [ ] runtime branch added in `stoa_cli/runtime_provider.py`
- [ ] CLI wiring added in `stoa_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `stoa model` but not `stoa setup`
