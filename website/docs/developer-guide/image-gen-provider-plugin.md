...
A bundled plugin is complete at this point. User plugins at `~/.stoa/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `hermes plugins enable <name>`).
...
```bash
export STOA_HOME=/tmp/hermes-imggen-test
