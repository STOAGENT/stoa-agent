...
```bash
stoa teams-pipeline token-health
stoa teams-pipeline token-health --force-refresh
```
...
```bash
stoa teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
stoa teams-pipeline maintain-subscriptions
stoa teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: STOA cron (recommended if you already run the STOA gateway)

STOA ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.stoa/scripts/`. First create the script:

```bash
mkdir -p ~/.stoa/scripts
cat > ~/.stoa/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec stoa teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.stoa/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
stoa cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/stoa-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=STOA Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=stoa
EnvironmentFile=/etc/stoa/env
ExecStart=/usr/local/bin/stoa teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/stoa-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run STOA Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now stoa-teams-pipeline-maintain.timer
systemctl list-timers stoa-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/stoa teams-pipeline maintain-subscriptions >> /var/log/stoa/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.stoa/.env` at the top of a wrapper script that crontab calls.
...
```bash
stoa teams-pipeline subscriptions   # should show expirationDateTime advanced
stoa teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
stoa teams-pipeline list
stoa teams-pipeline list --status failed
stoa teams-pipeline show <job-id>
```

### Replay a stored job

```bash
stoa teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
stoa teams-pipeline fetch --meeting-id <meeting-id>
stoa teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `stoa teams-pipeline maintain-subscriptions --dry-run`
- inspect `stoa teams-pipeline list --status failed`
...
- run `stoa teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `stoa teams-pipeline run`
...
- [ ] `stoa teams-pipeline validate` returns an OK snapshot
- [ ] `stoa teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (STOA cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
