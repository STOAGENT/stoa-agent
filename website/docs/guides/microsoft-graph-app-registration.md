...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.stoa/.env` at the end.
...
4. Fill in:
   - **Name:** `STOA Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `stoa-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the STOA app
New-CsApplicationAccessPolicy `
  -Identity "STOA-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict STOA meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "STOA-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.stoa/.env`:
...
STOA ships a Graph auth smoke-test. From your STOA install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.stoa/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `stoa gateway restart`.
