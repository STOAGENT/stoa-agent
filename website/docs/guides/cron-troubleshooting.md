...
description: "Diagnose and fix common STOA cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`stoa gateway` for foreground, or `stoa gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `stoa cron tick`.
...
```bash
date
stoa cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.stoa/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.stoa/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.stoa/.env` |
...
| `local` | Write access to `~/.stoa/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `stoa cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `stoa skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `stoa skills list`.
...
2. `~/.stoa/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `stoa cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the STOA config directory). Verify:
```bash
ls ~/.stoa/scripts/your-script.py   # Must exist
stoa cron edit <job_id> --script ~/.stoa/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `stoa skills install <skill-name>`.
...
Jobs are stored in `~/.stoa/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.stoa/cron/jobs.json
chmod 600 ~/.stoa/cron/jobs.json   # Your user should own it
```
...
```bash
stoa cron list                    # Show all jobs, states, next_run times
stoa cron run <job_id>            # Schedule for next tick (for testing)
stoa cron edit <job_id>           # Fix configuration issues
stoa logs                         # View recent STOA logs
stoa skills list                  # Verify installed skills
```
...
1. Run the job with `stoa cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
