...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.stoa/` (by default a single restore-point archive is written before apply, under `~/.stoa/backups/pre-migration-*.zip`; restorable with `hermes import`). |
