...
This tutorial walks you through setting up a Telegram bot powered by STOA Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **STOA Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.stoa/.env`

:::tip
A $5/month VPS is plenty for running the gateway. STOA itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by STOA Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u stoa-gateway -f
...
stoa gateway start
stoa gateway stop
tail -f ~/.stoa/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `stoa gateway install` to update the plist.
:::
...
```bash
stoa gateway stop && stoa gateway start
```
...
stoa pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.stoa/.env`:
...
Control how much detail the bot shows when using tools. In `~/.stoa/config.yaml`:
...
Customize how the bot communicates by editing `~/.stoa/SOUL.md`:

For a full guide, see [Use SOUL.md with STOA](/docs/guides/use-soul-with-stoa).
...
```bash
# From the CLI
stoa cron list          # View all scheduled jobs
stoa cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u stoa-gateway` (Linux) or `~/.stoa/logs/gateway.log` (macOS) |
| Cron job output | `~/.stoa/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.stoa/cron/jobs.json` |
| Pairing data | `~/.stoa/pairing/` |
| Session history | `~/.stoa/sessions/` |
