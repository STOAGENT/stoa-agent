---
sidebar_position: 6
title: "Use MCP with STOA"
description: "A practical guide to connecting MCP servers to STOA Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with STOA

This guide shows how to actually use MCP with STOA Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native STOA tool
- you want STOA to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect STOA to internal APIs, databases, or company systems without modifying STOA core

Do not use MCP when:
- a built-in STOA tool already solves the job well
...
- STOA remains the agent
- MCP servers contribute tools
- STOA discovers those tools at startup or reload time
...
If you installed STOA with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- STOA banner/status should show MCP integration when configured
- ask STOA what tools it has available
...
## WSL2: bridge STOA in WSL to Windows Chrome

This is the practical setup when:

- STOA runs inside WSL2
...
In this setup, STOA does **not** connect to Chrome directly. Instead:

- STOA runs in WSL
- STOA starts a local stdio MCP server
...
```text
STOA (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- STOA stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on STOA core browser transport
...
```bash
stoa mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
stoa mcp test chrome-devtools-win
```

Then start a fresh STOA session or run:
...
Once loaded, STOA can use the MCP-prefixed browser tools directly. For example:
...
If STOA runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start STOA from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start STOA from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in STOA:
...
So STOA will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want STOA to reason over a bounded workspace.
...
Now STOA can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing STOA core.
...
Because STOA now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
