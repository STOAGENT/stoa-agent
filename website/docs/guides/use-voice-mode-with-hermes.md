---
sidebar_position: 8
title: "Use Voice Mode with STOA"
description: "A practical guide to setting up and using STOA voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with STOA
...
- you want STOA sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in STOA.
...
## Step 1: make sure normal STOA works first
...
STOA supports both local and cloud speech stacks.
...
### If you use `stoa setup`

If you choose NeuTTS in the setup wizard, STOA checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. STOA transcribes and responds
...
- asking STOA to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full STOA loop.

## Tuning CLI behavior

### Silence threshold

If STOA starts/stops too aggressively, tune:
...
STOA stays a normal chat bot, but can speak replies.
...
- you want STOA to function like a portable research or ops assistant
...
STOA joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- STOA detects speech boundaries
- transcripts are posted in the associated text channel
- STOA responds in text and audio
...
1. get text STOA working
2. install `stoa-agent[voice]`
