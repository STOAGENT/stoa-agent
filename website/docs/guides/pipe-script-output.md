...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `stoa send`."
---

# Pipe Script Output to Messaging Platforms

`stoa send` is a small, scriptable CLI that pushes a message to any
messaging platform STOA is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | stoa send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `stoa
...
stoa send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | stoa send --to telegram:-1001234567890

# Send a file
stoa send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
stoa send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
stoa send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
stoa send --list

# Filter by platform
stoa send --list telegram
```
...
Any platform STOA ships adapters for works as a target:
...
`stoa send` resolves the message body in this order:

1. **Positional argument** — `stoa send --to telegram "hi"`
2. **`--file PATH`** — `stoa send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | stoa send --to telegram`

When stdin is a TTY (no pipe), STOA does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  stoa send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `stoa send` reuses your STOA config, the same script works on
any host where STOA is installed — no need to export bot tokens into
...
`stoa send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  stoa send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | stoa send \
...
```bash
./train.py --epochs 200 && \
  stoa send --to telegram "training done" || \
  stoa send --to telegram "training failed (exit $?)"
```
...
stoa send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(stoa send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `stoa send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `stoa send` calls
the platform's REST endpoint directly using credentials from
`~/.stoa/.env` and `~/.stoa/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `stoa gateway start` and retry.
...
The listing is built from `~/.stoa/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`stoa gateway
...
| Approach | Multi-platform | Reuses STOA creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `stoa send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`stoa send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `stoa send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `stoa send` shares with cron delivery.
