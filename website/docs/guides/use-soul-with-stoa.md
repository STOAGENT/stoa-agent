---
sidebar_position: 7
title: "Use SOUL.md with STOA"
description: "How to use SOUL.md to shape STOA Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with STOA

`SOUL.md` is the **primary identity** for your STOA instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want STOA to feel like the same assistant every time you talk to it — or if you want to replace the STOA persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm STOA should be
- what STOA should avoid stylistically
- how STOA should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who STOA is and how STOA speaks
...
STOA now uses only the global SOUL file for the current instance:

```text
~/.stoa/SOUL.md
```

If you run STOA with a custom home directory, it becomes:
...
STOA automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, STOA does not overwrite it
- if the file exists but is empty, STOA adds nothing from it to the prompt

## How STOA uses it

When STOA starts a session, it reads `SOUL.md` from `STOA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how STOA feels.
...
STOA already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How STOA should sound.

# Avoid
What STOA should not do.

# Defaults
How STOA should behave when ambiguity appears.
```
...
Then restart STOA or start a new session.
...
### I edited SOUL.md but STOA still sounds the same

Check:
- you edited `~/.stoa/SOUL.md` or `$STOA_HOME/SOUL.md`
...
### STOA is ignoring parts of my SOUL.md
