...
Opt-in toolset (not loaded in the default `stoa-cli` set). Add via `--toolsets video_gen` or enable it in `hermes tools` → Video Generation, which also walks you through picking a backend.
