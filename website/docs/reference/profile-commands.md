...
This page covers all commands related to [STOA profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `stoa profile` without a subcommand shows help.
...
```bash
stoa profile use <name>
```

Sets `<name>` as the active profile. All subsequent `stoa` commands (without `-p`) will use this profile.
...
stoa profile use default
```

## `stoa profile create`

```bash
stoa profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `stoa profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `stoa update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
stoa profile create mybot

# Clone config only from current profile
stoa profile create work --clone

# Clone everything from current profile
stoa profile create backup --clone-all

# Clone config from a specific profile
stoa profile create work2 --clone --clone-from work
```

## `stoa profile describe`

```bash
stoa profile describe [<name>] [options]
```
...
stoa profile describe researcher

# Set it explicitly
stoa profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
stoa profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
stoa profile describe --all --auto
```

## `stoa profile delete`

```bash
stoa profile delete <name> [options]
```
...
```bash
stoa profile delete mybot
stoa profile delete mybot --yes
```
...
```bash
stoa profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's STOA home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ stoa profile show work
Profile: work
Path:    ~/.stoa/profiles/work
...
```bash
stoa profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your STOA installation.
...
```bash
stoa profile alias work
# Creates/updates ~/.local/bin/work

stoa profile alias work --name mywork
# Creates ~/.local/bin/mywork

stoa profile alias work --remove
...
```bash
stoa profile rename <old-name> <new-name>
```
...
```bash
stoa profile rename mybot assistant
# ~/.stoa/profiles/mybot → ~/.stoa/profiles/assistant
...
```bash
stoa profile export <name> [options]
```
...
```bash
stoa profile export work
# Creates work.tar.gz in the current directory

stoa profile export work -o ./work-2026-03-29.tar.gz
```

## `stoa profile import`

```bash
stoa profile import <archive> [options]
```
...
```bash
stoa profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

stoa profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`stoa profile export` / `import` are still the right commands for
...
### `stoa profile install`

```bash
stoa profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `stoa -p telemetry`). |
...
stoa profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
stoa profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
stoa profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
stoa profile install ./telemetry/
```

### `stoa profile update`

```bash
stoa profile update <name> [--force-config] [--yes]
```
...
```bash
stoa profile info <name>
```

Prints the profile's distribution manifest — name, version, required
STOA version, author, env var requirements, the source URL/path, and
...
`stoa profile list` also shows the distribution name and version in a
`Distribution` column, and `stoa profile show <name>` / `delete <name>`
...
stoa profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
stoa profile install https://github.com/your-org/internal-assistant.git
```
...
`stoa_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
STOA version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host STOA can clone from.
3. Tell recipients to run `stoa profile install <your-repo-url>`.
...
```bash
stoa -p <name> <command> [options]
stoa --profile <name> <command> [options]
```

Global flag to run any STOA command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
stoa -p work chat -q "Check the server status"
stoa --profile dev gateway start
stoa -p personal skills list
stoa -p work config edit
```

## `stoa completion`

```bash
stoa completion <shell>
```
...
- `stoa profile <TAB>` — subcommands (list, use, create, etc.)
- `stoa profile use <TAB>` — profile names
- `stoa -p <TAB>` — profile names
