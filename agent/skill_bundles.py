...
Storage
-------
Bundles live in ``~/.stoa/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``STOA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``stoa bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under STOA_HOME.

    Honors ``STOA_BUNDLES_DIR`` for tests; falls back to
    ``<STOA_HOME>/skill-bundles``.
    """
    override = os.environ.get("STOA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_stoa_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `stoa bundles` CLI subcommand.
# ---------------------------------------------------------------------------
