...
Plugins built on STOA Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing STOA
...
:class:`~stoa_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors STOA' main config
...
handles every provider, fallback chain, and per-task override STOA
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`stoa_cli.plugins.PluginContext`
