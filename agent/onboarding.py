...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time STOA starts and finds ``~/.openclaw/``.

    Points users at ``stoa claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``stoa claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to STOA, run "
        "`stoa claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `stoa claw cleanup` (renames it to ~/.openclaw.pre-migration — "
