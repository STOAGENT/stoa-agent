...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.stoa/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: STOA ↔ models.dev
# ---------------------------------------------------------------------------

# STOA provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → STOA (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_stoa_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``stoa config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a STOA provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal STOA agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a STOA provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve STOA ID → models.dev ID
...
def get_model_info(
    ...
    Accepts STOA or models.dev provider ID.  Tries exact match then
