...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - stoa_home (str): The active STOA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.stoa``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "stoa").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'stoa memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], stoa_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'stoa memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``stoa_home`` is the active STOA_HOME directory path.
