...
Owns one Codex thread per STOA session. Drives `turn/start`, consumes
...
# STOA' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_STOA_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per STOA session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _STOA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("STOA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="stoa",
            client_title="STOA Agent",
            client_version=_get_stoa_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but STOA' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into STOA' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own stoa-tools callback we
            # auto-accept — the user already approved STOA' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map STOA approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    STOA returns 'once', 'session', 'always', or 'deny'.
...
def _get_stoa_version() -> str:
    """Best-effort STOA version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("stoa-agent")
