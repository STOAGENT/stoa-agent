...
from stoa_constants import get_stoa_home, get_skills_dir, is_wsl
...
DEFAULT_AGENT_IDENTITY = (
    "You are STOA Agent, an intelligent AI assistant created by STOA. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.stoa/kanban.db`. Your task id is in "
    "`$STOA_KANBAN_TASK`; your workspace is `$STOA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $STOA_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `stoa kanban <verb>` for board operations. Use "
...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "\n"
    "# Live data — never hallucinate\n"
    "For any question that depends on CURRENT, REAL, LIVE data, you MUST call a tool. "
    "Never guess, estimate, or describe a 'typical' value. The following classes of "
    "questions ALWAYS require a tool call BEFORE you respond:\n"
    "  - Current time, date, day-of-week, timezone — call `terminal` with `date` "
    "(POSIX) or `Get-Date` (Windows). Do NOT guess from the conversation timestamp "
    "or from training data.\n"
    "  - Current weather, temperature, conditions — use `web_search` or a weather tool.\n"
    "  - Current price of a stock, crypto, FX pair, commodity — use `web_search` or "
    "the dedicated market data tool if registered.\n"
    "  - Current status of a service, deployment, or remote system — use `terminal` "
    "or the appropriate diagnostic tool. Never speculate.\n"
    "  - Current contents of a file, directory, repo, or database — use `read_file`, "
    "`terminal`, or the database tool. Never recall from earlier turns if the data "
    "could have changed.\n"
    "  - Latest news, latest version of a package, latest commit, latest release — "
    "use `web_search` or `terminal git log`. Training-data cutoff means recalled "
    "answers are wrong by months.\n"
    "If the appropriate tool is not registered in your toolset for this session, say "
    "so explicitly ('I don't have a tool to check live X — please run "
    "`<concrete command>` and paste the output') rather than guessing. An honest "
    "'I cannot check that' beats a confident wrong answer every time. STOA users will "
    "screenshot wrong answers and post them; an honest 'I can't' is invisible."
)
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the STOA WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where STOA itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host STOA runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where STOA itself is running. The host OS, home, and cwd "
                f"of the STOA process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where STOA "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_stoa_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.stoa/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot STOA Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `stoa-agent` skill "
            "first. It has the actual commands (e.g. `stoa config set …`, `stoa tools`, "
            "`stoa setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: stoa setup, stoa setup tools, stoa setup terminal, stoa status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from STOA_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure STOA_HOME before loading SOUL.md: %s", e)

    soul_path = get_stoa_home() / "SOUL.md"
...
def _load_stoa_md(cwd_path: Path) -> str:
    ...
    if not stoa_md_path:
        return ""
    try:
        content = stoa_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = stoa_md_path.name
        try:
            rel = str(stoa_md_path.relative_to(cwd_path))
        except ValueError:
            pass
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    SOUL.md from STOA_HOME is independent and always included when present.
    ...
    # SOUL.md from STOA_HOME only — skip when already loaded as identity
    if not skip_soul:
