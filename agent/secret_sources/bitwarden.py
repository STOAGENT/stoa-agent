...
STOA pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.stoa/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<stoa_home>/bin/bws`` on
  first use.  STOA pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.stoa/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``stoa`` invocations don't
  hammer the API.
* Failures NEVER block STOA startup.  Missing binary, no network,
...
# In-process cache so repeated load_stoa_dotenv() calls (CLI startup,
...
def _stoa_bin_dir() -> Path:
    """Where STOA stores its managed binaries.  Profile-aware."""
    from stoa_constants import get_stoa_home

    return get_stoa_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<stoa_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _stoa_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``stoa secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _stoa_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="stoa-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "stoa-agent"})
    try:
...
# ---------------------------------------------------------------------------
# Public entry point — called from stoa_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_stoa_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `stoa secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `stoa secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `stoa secrets bitwarden setup` to install."
        )
        return result
