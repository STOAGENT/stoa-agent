...
as ``kind: backend``) or ``~/.stoa/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$STOA_HOME/cache/images/``, creating parents as needed."""
    from stoa_constants import get_stoa_home

    path = get_stoa_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$STOA_HOME/cache/images/``.
