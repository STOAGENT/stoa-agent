...
Every STOA request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to STOA Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=stoa-agent",
        "client=stoa-client-v<__version__>",
    ]

The version is sourced live from ``stoa_cli.__version__`` so it auto-aligns
...
``stoa_cli.__version__`` is the canonical source of truth.
"""
...
def _stoa_version() -> str:
    """Return the current STOA release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``stoa_cli`` cannot be imported (should
...
def stoa_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=stoa-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=stoa-client-v{_stoa_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=stoa-agent", stoa_client_tag()]
