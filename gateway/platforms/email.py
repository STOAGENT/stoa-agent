"""
Email platform adapter for the STOA gateway.

Allows users to interact with STOA by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "stoa-agent" "version" "{_stoa_version}" '
            '"vendor" "stoa-xyz" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        msg_id = f"<stoa-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
