...
There are two ways to add a platform to the STOA gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.stoa/plugins/` (or under `plugins/platforms/`
...
**zero changes to core STOA code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `stoa gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `stoa_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the STOA core.
...
And add it to the `stoa-gateway` composite:

```python
"stoa-gateway": {
    "includes": [..., "stoa-your-platform"]
}
```
...
## 12. Status Display (`stoa_cli/status.py`)
...
## 13. Gateway Setup Wizard (`stoa_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ stoa_cli/ toolsets.py \
  --include="*.py" -l | sort -u
