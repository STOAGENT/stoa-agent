...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_stoa_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /stoa prefix). A single regex
            ...
            # manifest (see `stoa slack manifest`). In Socket Mode, Slack
            ...
            from stoa_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/stoa$")

            @self._app.command(_slash_pattern)
            async def handle_stoa_command(ack, command):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: STOA handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own STOA session — matching the per-thread behavior channels already
                ...
                },
                {
                    ...
                    "elements": [
                        ...
                        },
                        {
                            ...
                            "action_id": "stoa_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "stoa_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "stoa_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "stoa_confirm_once": "once",
            "stoa_confirm_always": "always",
            "stoa_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "stoa_approve_once": "once",
            "stoa_approve_session": "session",
            "stoa_approve_always": "always",
            "stoa_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/stoa <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/stoa
        ...
        if slash_name in {"stoa", ""}:
            # Legacy /stoa <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from stoa_cli.commands import slack_subcommand_map
            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/stoa   ``).
        ...
        # "Running /cmd…" ack shown by handle_stoa_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/stoa <question>" must produce public replies so
