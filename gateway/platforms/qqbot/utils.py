...
def _get_stoa_version() -> str:
    """Return the stoa-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; STOA/<stoa_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; STOA/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    stoa_version = _get_stoa_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; STOA/{stoa_version})"
