...
The PID file lives at ``{STOA_HOME}/gateway.pid``.  STOA_HOME defaults to
``~/.stoa`` but can be overridden via the environment variable.  This means
separate STOA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting STOA_HOME."""
    home = get_stoa_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("STOA_GATEWAY_LOCK_DIR")
    if override:
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different STOA_HOME dirs).
    """
